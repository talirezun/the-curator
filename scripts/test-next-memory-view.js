...
// for the fold to be usable at all: the age and the size.
{
  ...
  ok('the closed summary carries the brief\'s AGE, live-ticking',
    ...
    // v3.67.2: the words are derived through the SHIPPED formatAge against
    // today's clock. The literal "1 week ago" pinned here went red on its own
    // on 2026-09-24, when a fixed fixture date aged into the next band — a
    // calendar failure, not a behaviour one.
    && idle.includes('class="mem-age-words">'
      + formatAge(Math.round((Date.now() - Date.parse('2026-09-10T00:00:00.000Z')) / 1000)) + '<'),
    idle.slice(0, 500));
