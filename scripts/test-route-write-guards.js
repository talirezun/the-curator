...
console.log('\n=== 6b. INVARIANT: every mutating route in these four files is guarded or explicitly exempted ===');
{
  ...
  auditRouteGuards('src/routes/config.js', {
    ...
    // 12 -> 13: POST /ui-state (v3.28.0), the durable record of the four
    // pieces of /next state whose loss is a correctness or trust failure —
    // the AI-Health privacy consent, the onboarding dismissal, the cutover
    // dismissal and the install-origin verdict. This tripwire fired on it,
    ...
    // reasoning.
    // 20 -> 21: PUT /context-window (v3.70.0) — the install-wide context
    ...
    guardClasses: [{
      ...
      exemptions: [
        ...
        { method: 'PUT', path: '/context-window', reason:
          'records two DISPLAY denominators (v3.70.0): the agent context window the Context view\'s window meter and the menubar widget are drawn against, and the owner\'s own ESTIMATE of the harness\'s share of it. NOTHING on any write path reads either: they are consumed only by src/brain/session-start.js when it composes a READ (the session-start report), never by ingest, compile, sync, the store or the MCP, so an in-flight ingest, sync or update cannot observe the change. Guarding it would be HARMFUL for /ui-state\'s reason: a 409 would fire while a long ingest runs and the meter would forget the window the owner just picked. The write is bounded to whole numbers by setContextWindowSettings()\'s allow-list (src/brain/config.js: an allowed window or a custom one within bounds, an estimate from 0 up to the window), which REFUSES rather than coerces and refuses to rewrite an unparseable settings file, so an unguarded PUT cannot put attacker-chosen content into .curator-config.json. The cross-origin guard in server.js still applies (test-session-start.js §8 proves a foreign-Origin PUT is 403).' },
