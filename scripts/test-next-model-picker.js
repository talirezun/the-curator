...
// there is `explainerMark(id, key)` from shared/explainer.js — injected into the
// render sandbox as the REAL kit (import-free of app.js), never a stub.
...
// explicitly; this section's data feeds it).
{
  for (const prov of POPULATED) {
    ...
    // v3.71.1: the "no second knob" sentence moved out of settings.js prose
    // into the block's ⓘ, which is now the `settings.build` explainer. Assert
    // the panel IS that explainer, byte for byte, and that it still says it.
    ok(block.includes(explainerHtml('settings.build')) &&
       /one model/i.test(block) && /nothing to set per job/i.test(block),
