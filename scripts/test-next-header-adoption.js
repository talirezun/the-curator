...
// under it, which is the whole defect.
{
  ...
  const PENDING = {
    ...
    //
    // settings.js IS GONE FROM THIS LIST TOO (v3.65.0), by the same protocol:
    // adopting turns the entry stale, the count check goes red, and whoever
    // adopted it deletes the line. Its one hand-rolled
    ...
    // component, the component offers no slot under it, and the structure
    // that would accept a floating paragraph no longer exists in the view.
    // Pinned INVERTED below, the way chat.js's was, so the adoption cannot
    // regress into a passing budget of 1.
