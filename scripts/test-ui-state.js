...
const STAYS = {
  ...
  'curator-memory-last-project-v1':
    'which project you last looked at in each domain (v3.48.0). Per-device positional state, the same class as curator-next-view and correct as per-device for the same reason: a laptop and a desktop have no business agreeing about which of five projects you had open. Its loss costs one click, it is validated on read (a bad shape degrades to no memory), and views/memory.js re-derives the selection by RECENCY whenever the remembered project is absent or no longer exists — so the fallback is not merely safe, it is the behaviour the feature had before the key existed.',
  'curator-context-window-v1':
    'which context window (200k or 1M tokens) step 4 of Project context divides by when it states what share of a window a session start takes (v3.67.0). A per-VIEWER denominator and nothing more: it changes one percentage on one screen, and nothing the project stores or any agent receives. Per-device for the reason curator-memory-folds-v1 is: two machines may run different models with different windows, and its loss costs one click. Validated on read (only 200k or 1m), every access wrapped, absent means 200k.',
  'curator-memory-folds-v1':
    'which of the Project-context folds you had open (v3.58.0; the view was called Agent memory until v3.62.0, and the KEY deliberately did not follow — renaming it would silently reopen every fold once, for every user, to buy nothing) — the standing brief and the session journal, both CLOSED by default. Per-device reading state of exactly the class curator-next-view is: a laptop and a desktop have no business agreeing about whether a section is expanded, and its loss costs one click on a chevron. It is validated on read (only the literal `true`, only under a known fold name, anything else degrades to "nothing open"), every access is wrapped because localStorage THROWS rather than returning null in a private window, and the default when it is absent is the designed default rather than a fallback. Moving it would mean a per-toggle write to the file that holds the API keys — the same churn the starred-models entry above declines.',
