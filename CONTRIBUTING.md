...
The Curator has an extensive battle-test suite (295 suites total — 272 OFFLINE
+ 15 LIVE_CI + 8 LIVE_LOCAL — thousands of assertions). One command runs them
...
Every screenshot in `README.md` and `docs/` is taken on a **synthetic demo workspace**, so
anyone can reproduce them and no personal wiki, machine name or repository ever reaches an image:

```bash
node scripts/screenshots.mjs                   # every shot, into docs/images/
node scripts/screenshots.mjs --list            # the shot names
node scripts/screenshots.mjs --only chat,sync  # just these
node scripts/screenshots.mjs --out /tmp/x      # write somewhere else to review first
node scripts/demo-domain.mjs <empty-dir>       # just the demo wiki, to run the app against by hand
```

- `scripts/demo-domain.mjs` writes two hand-authored domains (Early Computing, 23 pages;
  Night Sky, 6 pages) and five saved conversations — no LLM call, fixed ids. With
  `--workspace <empty user-data dir>` (and both `CURATOR_TEST_*` seams pointed at the two
  directories) it also writes, through the app's own store functions: a project
  (`exhibit-site`) with a brief and two canonical documents, handoffs from two tools
  (Claude Code and Antigravity, one shared scope with a kept `previous.md`), their journals, a
  content-free MCP usage log, one deleted handoff, project and domain in the trash, and a
  Personal Sync repo whose "GitHub" is a local bare repository labelled
  `github.com/example/my-brain`. The machine is `demo-mac-4d3e2f`. A fake clock spreads the saves
  over a week; only the ages follow the current time (pin them with `--now <ISO time>`).
- `scripts/screenshots.mjs` seeds a fresh workspace under `/tmp/curator-demo-XXXXXX`, starts
  `src/server.js` isolated (both test seams, `HOME` pointed at an empty fake home, every
  provider/GitHub credential stripped, fake keys — or none, for the shots of the unset state),
  drives the Chrome/Chromium already on the machine through the visual harness's own launcher
  and CDP client (`scripts/visual/`), and captures each shot at 1280 CSS px wide ×1.25. The
  images the README shows as a light/dark `<picture>` are taken in both themes. It refuses to
  write an image if the page shows this machine's host name, user name, home path or checkout
  path. With no browser it prints a skip and exits 0.
- It is not in `npm test`. Re-run it when a view changes shape, **look at every image**, and
  update the alt text where the image is used (`git grep images/<name>.png`).
- `images/the-curator-demo-2026.gif` is kept in the repository but no longer referenced: it
  predates the v3.64 redesign and cannot be regenerated by this script.
