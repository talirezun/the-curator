...
allowed-tools: mcp__my-curator__list_domains mcp__my-curator__get_index mcp__my-curator__get_graph_overview mcp__my-curator__get_tags mcp__my-curator__search_wiki mcp__my-curator__search_cross_domain mcp__my-curator__get_node mcp__my-curator__get_connected_nodes mcp__my-curator__get_backlinks mcp__my-curator__get_summary mcp__my-curator__get_raw_source mcp__my-curator__get_working_state mcp__my-curator__get_project_context mcp__my-curator__list_projects mcp__my-curator__compile_to_wiki mcp__my-curator__scan_wiki_health mcp__my-curator__fix_wiki_issue mcp__my-curator__scan_semantic_duplicates mcp__my-curator__get_health_dismissed mcp__my-curator__dismiss_wiki_issue mcp__my-curator__undismiss_wiki_issue mcp__my-curator__save_working_state mcp__my-curator__save_project_brief mcp__my-curator__save_foundation
---

# My Curator — second brain playbook

This skill is the canonical playbook for working with the user's **My Curator** second brain through the **my-curator MCP**. The MCP exposes 24 tools — 14 for reading (the wiki graph, plus your own prior working state, the projects it belongs to, and — since v3.59.0 — a project's canonical foundations), and 10 in the health/authoring group, of which 7 actually mutate something (`compile_to_wiki`, `fix_wiki_issue`, `dismiss_wiki_issue`, `undismiss_wiki_issue`, `save_working_state`, `save_project_brief`, `save_foundation`). This playbook tells you how to use them well, in the order that produces the best results.

**Read the tool list you were actually given, not this paragraph.** Older Curator builds register fewer tools — `get_raw_source` needs v3.5.0+, the two working-state tools need v3.17.0+ (below which the count is 18), `list_projects` / `save_project_brief` need v3.48.0+ (below which it is 20, and every project is simply the domain it lives in), and `get_project_context` / `save_foundation` need v3.59.0+ (below which the count is 22 and there is no foundations tier at all). A tool that is absent is simply unavailable; everything else here still applies.
...
4. **If a project is in play, its context already names its wikis.** `get_project_context`
   and `get_working_state` return `knowledgeDomains` — the wikis that project draws on —
   beside `knowledgeDomainsDefaulted`, which says whether the owner chose the list or it
   fell back to the project's own domain. Search there rather than asking again. (v3.65.0;
   an older server omits both fields, and an absent list means "not told", never "none".)
5. **Domains are siloed.** Don't try to write a link from one domain to a page in another. If the user wants cross-domain synthesis, that's a `search_cross_domain` reading task — not a writing task.

### §3.1 — Shared Brain mirrors, in one paragraph

Some domains in `list_domains` are **Shared Brain mirrors** — named `shared-<slug>` (e.g. `shared-cohort`). They are local **read-only** copies of a collective wiki. **Reading one is unrestricted**: all fourteen read tools work normally (seventeen, counting the three read-only scans that sit in the health block), and answering *"what does our cohort wiki say about X?"* by traversing a mirror is exactly what it is for. **Writing to one is refused**: all seven mutating tools check the domain's `CLAUDE.md` for `readonly: true` and return an error telling you to call the tool on the user's personal opted-in domain instead, then run **Push contributions** from the app's **Shared Brain** view. The pure scans — `scan_wiki_health`, `scan_semantic_duplicates`, `get_health_dismissed` — are not guarded, correctly, because they only read. **Whenever a `shared-*` domain is in play, open [shared-brain.md](shared-brain.md)** for the contribution flow, the verbatim refusal text and the dialogue scripts.
...
| `save_foundation` | Write or replace ONE canonical document (tier 0, v3.59.0) — architecture, decisions, conventions, roadmap | **Only when the user explicitly asks.** Never on your own initiative — same rule as `save_project_brief` |

**`save_project_brief` and `save_foundation` are the two tools here you must not reach for helpfully.** Both write documents future sessions are told to treat as authoritative context — the brief as the user's own advance instructions, a foundation as this project's canonical reference — so writing either unasked means editing material other sessions will trust without re-litigating. `save_foundation` additionally **refuses outright** without `commissioned_by_owner: true` on the call — there is no soft failure mode to route around; the tool will not write without it. Both replace their target WHOLE: for the brief, read the current one first and send it back with your changes folded in, or the rest is destroyed, and unlike a handoff there is no journal behind tier 1 to recover it from; for a foundation, the same applies — a partial resend is a partial document. Neither tool records itself as your own initiative: the brief's provenance comment and a foundation's `authoredBy` both name the agent and the harness, and later readers are told an agent wrote it, on the owner's instruction.

**A project's foundations can come from several places, and `save_foundation` writes only the ones kept here.** Each document is written here, copied in from a folder, or mirrored from a folder or a GitHub repository — one project may hold all of them. A save to a **mirrored** document (`source.kind: "repo"`) is refused: the source, not an agent, is the author there, and the fix is a refresh (`repo_root` on a `save_working_state` call, or **Refresh** in the app). A save of a **new** document is fine in any project, including one that also mirrors a repository.
