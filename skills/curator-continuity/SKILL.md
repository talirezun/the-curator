...
- **Before saving under a scope the brief sets no rule for, check who saved there last.** If the newest save in that scope was made by a **different tool** (the `harness` on its index row or journal line), save under a scope named for your own tool instead — `claude-code`, `antigravity` — and say so in your reply. Reading that scope is always fine; only your save moves. If the save reply comes back with `overwrote` set, you have already replaced the other tool's handoff: its text is gone and the Journal keeps only its headline, so tell the user, and save under your own scope from then on.
