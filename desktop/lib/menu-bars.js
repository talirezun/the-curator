...
// ── THE SESSION-START METER WAS REMOVED WITH ITS LINE (v3.74.0) ────────────
//
// `renderGutterMeter` drew the context-window meter for the menu's Session
// start line (v3.70.0). The line left the menu on the maintainer's decision —
// it is a planning reading about the context CONFIGURATION, its home is the
// app's Context step ④ (which keeps it), and it cost a full
// `get_project_context` run on every tray refresh — so the renderer had no
// consumer and went with it, rather than staying as drawing code nothing
// calls.
