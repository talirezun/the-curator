...
// v3.64.0 (package K) — the injection-defence prose moved DOWN into
// `src/brain/` so an in-process caller (chat.js) can reuse the exact wording
// without reaching into `mcp/`. Moved byte-identical; this file re-exports
// nothing new. `BRIEF_OWNER_PROVENANCE`/`BRIEF_STANDING_RULES`/
// `BRIEF_IS_OWNER_AUTHORED`/`BRIEF_IS_COMMISSIONED_PREFIX`/
// `BRIEF_UNTRUSTED_REASON` and `FOUNDATIONS_ARE_DATA`/`skeletonsArePrompts`
// moved too but are used only inside the new module's own
// `briefAuthorityNote`/`composeContentIsData` — nothing here calls them
// directly, so they are not re-imported.
import {
  CAVEAT_BODY,
  JOURNAL_IS_HISTORY,
  REJECTIONS_LEGEND,
...
function contextReport(out, project) {
  ...
  if (f.manifestError && f.manifestErrorCode === 'manifest-newer') {
    // v3.68.1 — a newer app's manifest is not broken; never tell the user to fix or delete it.
