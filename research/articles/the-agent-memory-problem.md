...
The MCP exposes seventeen dedicated tools *(update, 26 September 2026: now 24 — see the note below)* — ten for reading the wiki and seven for writing to it. The read tools are not simple keyword search. They are **graph-native**: topology overviews, multi-hop neighbourhood traversal, bidirectional backlinks, cross-domain search across every knowledge domain you have built simultaneously. The write tools allow you to save research findings back into the wiki directly from a conversation — compiling what was discussed, updating existing pages, adding new connections.
...
> **Update, 26 September 2026.** This thesis turned out to be half right, and the half that was wrong shaped the rest of the project. The wiki is a good memory for *knowledge* — what you have read and concluded. It is the wrong store for what a coding agent needs between sessions: where the work stands, what was decided, what already failed. A wiki page accumulates. Every write adds and nothing is dropped, so "blocked on the login bug" could never be marked as no longer true. Building the coding-agent case below therefore produced a **separate working-state layer** (v3.17.0) that overwrites instead of accumulating. Each project has a standing brief that its owner writes, or explicitly asks an agent to write, a handoff per workstream and machine, and a journal of saves. Since v3.59.0 it also carries the project's canonical documents. The rule is *state supersedes, knowledge accumulates* ([working-state docs](../../docs/working-state.md#what-it-is-not)). The bridge grew from 17 tools to 24 to carry it ([MCP guide](../../docs/mcp-user-guide.md#what-it-does)). [Article 7](./the-handoff-writes-itself.md) and [Article 8](./where-your-context-lives.md) tell that story. How far it is proven: one project has been handed back and forth between Claude Code and Antigravity, on one Mac and across two Macs through Personal Sync, in both directions, in my own live tests on 25 and 26 September 2026. Other harnesses (Codex, Cursor, opencode) have not been tested in a live handover, and neither has several people sharing one project.
...
> **Update, 26 September 2026.** The collective layer was built, and differently from this plan. **Shared Brain** shipped as an opt-in beta in v3.0.0-beta (May 2026), and there is no hosted service. Each group uses its own private GitHub repository. Each contributor's own Curator summarises the pages they changed in the domains they opted in, the group's admin runs synthesis on their own machine, and every member pulls the result back as a read-only domain that agents can query over the bridge ([Shared Brain architecture](../../docs/shared-brain.md#3--architecture)). The Cloudflare Workers and R2 path is still planned and has not shipped ([roadmap](../../docs/shared-brain.md#7--roadmap)). The structured pilot with a real cohort, which is the gate to leaving beta, has not started. [Article 5](./the-shared-brain-thinking-together.md) and [Article 6](./neural-network-of-your-own-knowledge.md) cover it.
...
[The Curator](https://github.com/talirezun/the-curator) is free. It is open-source. It installs in a single terminal command. It runs entirely on your hardware. With the exception of Google Gemini API used for ingestion and health checks, it requires no external database and no ongoing service fee. *(Update, 26 September 2026: on a Mac it also comes as a packaged app; ingest and AI health can run on Gemini, Anthropic or OpenRouter; and the MCP bridge and the working-state layer call no model at all.)* And through [My Curator MCP](./from-graph-to-intelligence-my-curator-mcp.md), it gives any MCP-compatible AI model access to a graph-native memory layer built from the most durable format that exists — plain text files.
