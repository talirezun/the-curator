...
The memory layer is written by agents, over MCP, while you are doing something else entirely — and increasingly by more than one agent, in more than one harness, on more than one machine. Before the widget, the only way to see any of that was to open the app and navigate to [the Agent Memory view](../../docs/working-state.md#6-what-the-app-writes-and-what-it-does-not).
