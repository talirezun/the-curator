...
[The Curator](https://github.com/talirezun/the-curator) is a local, open-source application, MIT-licensed, currently at **v3.0.6**, built on a Node.js/Express backend with a vanilla JavaScript frontend — no build step, nothing to compile — and an LLM abstraction layer that runs on Google Gemini or Anthropic Claude, your choice. *(Update, 26 September 2026: The Curator is at v3.76.1 as this note is written, and OpenRouter became a third provider in v3.15.0.)* If you want to follow along from zero, the [User Guide](../../docs/user-guide.md) walks through installation, your first ingest, and daily use in plain language.
...
> **Update, 26 September 2026.** The count above is out of date. The bridge now exposes **24 tools**: 14 that read and 10 in the write block, of which seven change files on disk ([MCP guide](../../docs/mcp-user-guide.md#what-it-does)). The additions are `get_raw_source` (v3.5.0) for the original document behind a summary, and six tools for a coding agent's working state: `get_working_state` and `save_working_state` (v3.17.0), `list_projects` and `save_project_brief` (v3.48.0), and `get_project_context` and `save_foundation` (v3.59.0).
...
> **Update, 26 September 2026.** One refinement to the four layers. A compiled wiki turned out to be the right memory for what an agent should *know*, and the wrong one for what it needs to *resume* work: where the task stands, what was decided, what already failed. A wiki accumulates and cannot mark a fact as no longer true. So the next thing built, after Shared Brain, was a separate working-state layer that overwrites instead ([working-state docs](../../docs/working-state.md#what-it-is-not)). [Article 7](./the-handoff-writes-itself.md) and [Article 8](./where-your-context-lives.md) cover it.
...
What hasn't happened yet, and what the project's own [roadmap](../../docs/shared-brain.md) names as the actual gate to a v3.1.0 general-availability release, is a structured pilot with a real cohort. *(Update, 26 September 2026: v3.1.0 shipped as unrelated infrastructure work, so general availability no longer has a version number. The pilot has still not started, and Shared Brain is still an opt-in beta as of v3.76.1. See the [roadmap](../../docs/shared-brain.md#7--roadmap).)* Everything up to this point has been verified by tests, simulations, and solo or paired dogfooding between me and a handful of early testers. The Vanguard MBA cohort remains the natural first audience for that pilot, and it's the piece I'm most looking forward to writing about next, because it's the one chapter of this story that no amount of careful engineering can substitute for.
...
> **Update, 26 September 2026.** None of this has shipped, and the version numbers no longer apply: v3.1 and v3.2 were used for unrelated work, and the app has since moved to v3.76. General availability is still gated on the cohort pilot, which has not started. The Playwright wizard pass is run for each release rather than kept in the repository, because the auto-updater would otherwise install Playwright on every source-install machine. Cloudflare R2 is still planned for after GA, and enterprise mode is further out ([roadmap](../../docs/shared-brain.md#7--roadmap)).
...
> **Update, 26 September 2026.** That article has not been written, because the pilot has not happened yet. The next two articles went somewhere I did not expect in July: [Article 7](./the-handoff-writes-itself.md) and [Article 8](./where-your-context-lives.md) are about the working-state layer that coding agents read and write between sessions. The question in the paragraph above is still open.
