...
> **Update, 26 September 2026.** When I wrote this, cross-harness handover was a property of the design rather than something I had measured. Two harnesses have now been tested live, by me, on my own projects: **Claude Code and Antigravity** handed one project back and forth on one Mac on 25 September (Claude Code → Antigravity → Claude Code, each tool saving under its own scope, unprompted), and across two Macs through Personal Sync on 26 September, in both directions. Codex, Cursor and opencode have **not** been tested in a live handover; opencode has only been measured headless, where it switched the continuity skill on by itself in 4 of 4 runs. The checklist that made the two-tool, two-Mac case work is in [the user guide](../../docs/user-guide.md#13d-working-with-several-agent-tools-and-several-computers--the-setup-checklist).
...
> **Update, 26 September 2026.** "There are no hooks" is no longer true. Since version 3.63.0, `my-curator install-hooks` can wire optional hooks that hand the agent its context at the start and ask for a save at the end. They ask; they still never save on the agent's behalf, so capture remains advisory. Measured on Claude Code on 20 September, in headless mode only: the start-of-session hook delivered the context in every run, and the end-of-session ask never fired in that mode. Antigravity's hooks are built from its documentation and had not been measured when this note was written. [The hooks table](../../docs/user-guide.md#hooks-what-they-can-do-on-your-harness-and-what-they-cannot) has the per-harness state.
...
> **Update, 26 September 2026.** What now tells an agent in another harness to save is a short block the app copies for each project, pasted at the top of the file that harness loads every session (`CLAUDE.md`, `AGENTS.md`). Measured headless on Claude Code on 25 September, Sonnet 5 read and saved in 8 of 8 runs, every save in its own tool's scope; the small Haiku 4.5 saved in 5 of 8. In my live tests Antigravity, reading the block from `AGENTS.md`, read and saved unprompted. Without that file it still saved — but under a shared name, overwriting Claude Code's handoff, which is why a save that replaces another tool's handoff now says so and keeps the replaced text. One gap surfaced on 26 September and was closed the same day: a conversation already open did not re-read after the other Mac had saved. The block now tells it to on "continue" (Sonnet 5: 0 of 8 re-read before the change, 8 of 8 after; no measurable change on Haiku 4.5). Still unmeasured: Antigravity's hooks, Haiku beyond these small runs, and every harness other than Claude Code and Antigravity in a live handover.
