...
Only the **cohort admin** can revoke a contributor. Two gates stand in front of the operation, and both are checked on every request:

1. The request must carry the connection's `admin_token`. It is generated at brain setup, shown once, stored only on the admin's own machine, and never shared. The comparison is constant-time over sha256 digests. A connection that holds no admin token — which is every ordinary contributor's connection — is refused outright: `403` with `code: "no_admin_token"`. There is no way to obtain one from a running app.
2. The request must carry the literal confirmation string `REVOKE-<fellow_id>` — the full UUID, not the short id. This is the same pattern GitHub uses for repo deletion: it forces the admin to consciously target one specific contributor.

> **This was not fully true before v3.43.0, and the correction is worth stating plainly.** Gate 1 was
> only ever as strong as the weakest way to *obtain* an admin token — and until v3.43.0 there was a
> way. `POST /api/sharedbrain/:id/admin-token/rotate` issued a fresh admin token to **any** connection
> that asked, with no proof of possession at all, and the Shared Brain card offered the button to every
> connection that was not read-only. A live end-to-end test walked the whole path: a plain contributor
> minted an admin token, read the admin's `fellow_id` out of `GET /:id/members`, and revoked the admin.
> Since v3.43.0 that endpoint **rotates** an existing token and never provisions one: it requires the
> CURRENT token through the same gate as revocation, and refuses a connection that has none.
>
> **Consequence for pre-v3.0.5 admin connections**, stated rather than left to be discovered: an admin
> whose connection predates admin tokens can no longer mint one from the card. Re-run the brain-setup
> wizard, which issues a fresh admin token and saves it with the connection.
...
5. **Appends an audit entry** — `state/revocations.jsonl` gains one line: `{"revoked_at": "<ISO>", "fellow_id": "<uuid>", "by_admin_token_hash": "sha256:<salt>:<digest>", "contributions_deleted": N, "pages_deleted": M, "pages_rebuilt": R, "rebuild_ok": true|false, "erasure_complete": true|false, "contributions_failed": N, "pages_failed": N, "digest_failed": true|false, "pages_rebuild_failed": N, "state_reset_failed": true|false, "revocation_id": "<uuid>"}`. Failures are recorded as **counts and booleans only** — the audit log still contains no real names and no contribution content, and deliberately no provider error text either. **`by_admin_token_hash` is SALTED per record since v3.43.0** (`sha256:<16-byte salt>:<sha256(salt + ":" + token)>`). This file sits in the shared repo and every contributor can read it, and until v3.43.0 it held a bare unsalted digest — an offline verification oracle against a token the schema permits to be any 16-character string, i.e. a hand-set `cohort-admin-2026` was recoverable by anyone in the cohort. An admin holding the token can still verify any record by recomputing the digest with that record's own salt; what is gone is precomputation, and — deliberately — the ability to tell from the log alone that two revocations were performed by the same admin. Admins can review revocation history — including attempts that *recorded* failures — without exposing PII. **Note the limit, because it matters for an audit trail:** this line is written near the end of the run, so it exists only for revocations that got that far. A run that aborts earlier writes **no** audit line at all — that covers an unreadable collective-page listing (step 3), and a failure to initialise storage, to write the in-progress marker, or to list the contributor's submissions. Those attempts leave `state/revocations.jsonl` looking as if they never happened; record them yourself from the API response.
...
| 4 | **Do you have an admin procedure for revocation requests?** When a contributor leaves (graduates, changes jobs, requests removal), someone needs to run the revoke operation. The Curator UI surfaces this in the **Shared Brain** view → the connection's card → **"Admin controls — admin token & contributor revocation"** (visible only when the connection holds an admin token). Document who in your org has the `admin_token` and the procedure. |
