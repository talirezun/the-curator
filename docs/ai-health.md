...
> **Interface note — the per-row ✨ Ask AI button was retired along with the pre-redesign interface.** Phases 1 and 2 below describe a button that used to appear on an individual broken-link or orphan row in the pre-redesign shell (`/old`, deleted in v3.41.0). That control was never ported to the redesigned interface, which since the v3.9.0 cutover is what you get at `/`. There, the LLM-backed actions are the three **✨** buttons in the **Quick maintenance** bar — *Fix broken links*, *Rescue orphans*, *Find duplicate pages* — which do the same work in a reviewed batch with a preview, and are described in the bulk sections further down. Read Phases 1 and 2 for what the AI is asked and what leaves your machine; that part is identical.

When Wiki Health finds a broken link it could not match algorithmically, the row is tagged **Review** and — in the now-deleted pre-redesign interface — a **✨ Ask AI** button used to appear (only if you have an API key configured in Settings).
...
An **orphan** is an entity or concept page with zero incoming `[[wikilinks]]` — content you captured that the graph doesn't yet know about. The Health scan has always listed orphans as review-only. From v2.4.4 through v3.40.0, each orphan row in the pre-redesign interface got a **✨ Ask AI** button (when an API key is configured); that interface was deleted in v3.41.0, and the current interface offers the batch **✨ Rescue orphans** action instead.
...
If the active model has no published price, `estimateUsdCost` returns `null` and the estimate/plan payload additionally carries `priceKnown: false` and a `costNote` explaining why, rather than silently substituting a wrong number. `/next`'s `costReadout` (in [`src/public/next/views/domains.js`](../src/public/next/views/domains.js)) renders `costNote` verbatim in that case, falling through to the server-supplied sentence whenever `estimatedUsd` is null. (Through v3.40.0 the pre-redesign shell's `formatHealthCost` in `src/public/app.js` did the same thing — both frontends read `costNote` the same way; that shell was deleted in v3.41.0, leaving `costReadout` as the only renderer.) This covers every pre-run confirm dialog (semantic-duplicate scan, broken-link fix, orphan rescue) **and** every post-run readout — the broken-link and orphan-rescue plan previews ("Planning cost: …") and the semantic-dupe scan's "Done" summary all read `costNote` the same way, so an unpriced model shows the real sentence there too, not a blank string. There are THREE states here, not two, because a FREE model's price is **known** and it is zero. A free model returns `estimatedUsd: null` with `priceKnown: true` and a note saying it is free — deliberately not `estimatedUsd: 0`, because a numeric zero made the now-deleted `formatHealthCost` print `$0.0000`, the exact string this project's currency formatter exists to prevent (a rule `costReadout` still honours, for the same reason). An unpriced model returns `priceKnown: false`, which still means *we were not told*. The one deliberate exception is `/next`'s per-domain quick-action button badge: a full sentence would break that pill's layout, so `costReadout(est, { compact: true })` renders a short **"free"** or **"cost unknown"** according to `priceKnown`, instead of the server's longer note. `/next` does not currently display an actual post-run cost figure anywhere (the scan result stores `cost` but no view renders it) — a pre-existing gap in that frontend, unrelated to pricing coverage, not tracked here.
...
A one-time disclosure modal summarises this the first time you launch an ✨ AI action in a browser (a Quick maintenance button in the current interface; through v3.40.0 the pre-redesign shell's per-row **Ask AI** button read the same acknowledgement key, so a user was never asked twice regardless of which interface they used). Accepting it stores the acknowledgement in `localStorage` under the key `curator-ai-health-disclosure-seen-v1`.
...
- **Remove every provider key** in Settings → Disconnect. The ✨ AI actions disappear from the Wiki health panel on the next scan. Any one key left in place is enough to keep them available.
