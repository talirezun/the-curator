...
- **Fallback chain** (re-verified live and reordered forward-in-time in v3.0.15 — the prior chain's `gemini-1.5-flash` / `gemini-1.5-flash-latest` rungs both 404'd): `gemini-3.1-flash-lite` → `gemini-2.5-flash` (`gemini-3.5-flash-lite` was removed 2026-08-26 as strictly dominated)
...
- **Fallback chain**: Sonnet only. A 2026-08-24 live probe found **4 of the 5 previous rungs returned 404** — the entire `claude-3-5-haiku` / `claude-3-*-sonnet` generation is retired from the direct API. There is no live Haiku cheaper than the default to fall back to, so the chain escalates straight to Sonnet (`claude-sonnet-5` first, which is both newest and cheapest of the three). See [model-lifecycle.md](model-lifecycle.md).
...
- **Explicit prompt caching (v3.0.16).** Unlike Gemini, Anthropic caching is opt-in per call — the caller marks a `cache_control: { type: 'ephemeral' }` breakpoint on the prefix it wants cached. `buildAnthropicUserContent(userPrompt, cachePrefixChars)` in `llm.js` places that breakpoint at `cachePrefixChars`, but only when the caller (`ingestMultiPhase`) opts in, gated by two independent conditions:
  ...
  - **Size gate** — `ANTHROPIC_CACHE_MIN_PREFIX_CHARS = 16,000` characters (~4,000 tokens). This is the real cache-eligibility floor on `claude-haiku-4-5`. (An earlier version cited a 2,048-token floor on the `claude-3-5-haiku` rungs, which were removed in 2026-08 after a live probe found them 404. A **later correction claiming no reachable model has a lower floor was wrong in the opposite direction**: Sonnet 5 / 4.6 / 4.5 — all three current rungs — have a **1,024-token** minimum, four times *lower* than Haiku 4.5's 4,096. Consequence today is benign, because `buildAnthropicUserContent` takes no model argument and so applies the single 16,000-char floor regardless — but that means a cacheable Sonnet prefix between ~4k and 16k chars is skipped. Worth revisiting if Sonnet ever becomes a primary rather than a fallback.) A prefix below the active model's minimum is silently NOT cached by the API (no error, no write charge), so being wrong in either direction is cheap — but a breakpoint on a too-short prefix is simply a no-op, so `llm.js` enforces the floor itself rather than relying on the API to no-op gracefully.
...
OpenRouter is an **aggregator**, not a vendor: one key and one wire format in front of models from many companies. That difference shapes everything below.

- **Default model**: `upstage/solar-pro4` — pinned after a live pass against this repo's real outline prompt (nine runs per candidate). It returned raw-parseable JSON in 9 of 9 runs, spent no hidden reasoning tokens, and planned a median of 23 outline pages. At $0.09/$0.36 per 1M tokens it is roughly nine tenths of the price of `gemini-2.5-flash-lite`; it is pinned on coverage and JSON cleanliness rather than on price (the affordability claim was re-measured on 2026-09-16 and had to be cut — see `src/brain/llm.js:289-296`).
- **JSON mode**: `response_format: { type: 'json_object' }` is sent, *and* the request declares that an upstream must not silently drop unsupported parameters. That second half is the one that matters: a dropped `response_format` does not error, it returns prose, which surfaces as a parse failure several layers away with no indication of the real cause.
- **No provider substitution.** Every request forbids OpenRouter from re-routing to a different upstream, and the request shape that would allow *model* substitution is never sent. So the model you picked is the model billed, and the only substitution that can happen is our own fallback chain below.
- **Fallback chain**: exactly one rung — `ibm-granite/granite-4.0-h-micro`. It was measured equally clean (9 of 9 raw-parseable JSON) and is **cheaper** than the default it backs up ($0.017/$0.112), so the net can never cost more than the thing it replaces; its measured weakness is coverage, not correctness (a median of 9 outline pages against 23). A free model is deliberately **not** a rung — see the note on free models below.
- **Free models can build a wiki, but nothing is promised.** `minimax/minimax-m3:free` is offerable for ingest and measured the widest coverage of the three (15–40 pages, median 21), with one of nine runs needing the `jsonrepair` fallback. Free ids draw on a **shared upstream pool**, so availability is not guaranteed: over one ten-round poll it answered every time while three of its free siblings answered none of theirs and reported being rate-limited upstream throughout. That is why the pinned default is a paid model — a default is what runs when the user has chosen nothing, and it must not be a shared queue that can stall a forty-call multi-phase ingest. This document deliberately prints **no rate-limit figure**: the caps are OpenRouter's to change, and the app reports the real ones it reads back from your own key.
- **Free models have no price, and that is the point.** `getModelPrice()` returns `null` for a free id rather than `{input: 0, output: 0}`, because a zero price is *truthy* and would silently make the batch-ingest budget cap inert. The queue therefore reports a free batch as costing nothing rather than showing `$0.00` (see §10g.8).
- **No explicit prompt caching.** The §1b prefix reorder still applies — the batch prompt is built the same way for every provider — but no cache breakpoint is sent on this path, because that mechanism is Anthropic-specific. Whether an upstream caches implicitly is not something this app measures, so no saving is claimed here.
- **There is no per-model output-cap table for OpenRouter, and that is deliberate.** Anthropic has one because its API rejects a `max_tokens` above the model's ceiling, so an unrecognised id must clamp to a conservative default. OpenRouter has none: an aggregator id routes over upstream hosts that can change, so a ceiling frozen into our source would be a snapshot of a fact that can move *without the id changing*. Each offerable entry carries the ceiling it was measured with instead, and an id that publishes no ceiling is refused rather than given a guessed one.
- **Usage accounting follows the *Gemini* convention, not Anthropic's** — the prompt-token count **includes** cached tokens, so the normaliser subtracts them. Getting this wrong double-counts silently. See [architecture.md](architecture.md#srcbrainllmjs).

All three providers go through the same retry path for 429 (rate limit) and 503 (overloaded) — up to 4 attempts with exponential backoff. Users see "Service busy — retrying in 9s… (attempt 2/3)" during the backoff — this is the **retry surface, not a bug**, and ingests routinely succeed after the wait.
...
Those figures are for one model, and the spread across the models an ingest can run on is wide. OpenRouter's pinned default (`upstage/solar-pro4`, $0.09/$0.36 per 1M tokens) sits just under the Gemini figures above, its fallback rung (`ibm-granite/granite-4.0-h-micro`, $0.017/$0.112) cheaper again, and one offerable OpenRouter model is free outright. Claude Haiku 4.5 is materially more expensive — measured on an identical 4-document batch against the same 3,336-page domain, $0.127088 against Gemini's $0.017225 (§10g.8). The pre-spend estimate always prices the model you are actually on, never the figures printed here.
...
71 offline assertions covering atomic-write contract (including SIGKILL simulation), EXDEV defense, trunk-page detector, write registry, and file-based lock. Run with: `node scripts/test-beta8-stress.js`.
...
46 offline assertions specifically targeting the v3.0.1-beta.11 changes:
...
**500 offline assertions** (grown from 204 in v3.0.16). The v3.0.16 portion covers the §1b prompt-assembly changes end-to-end:
...
- Token usage (`opts.onUsage`) is normalised to one common shape on every provider — Gemini and OpenRouter both report a prompt count that *includes* cached tokens and are made to subtract them, Anthropic reports one that excludes them — and never affects the call's outcome, even when the callback throws
...
Together the suites described in this section run **773 offline + 174 live-LLM = 947 assertions** that exercise the pipeline at every level (write + read) — 9.1 (94) + 9.2 (71) + 9.5 (46) + 9.6 offline (62) + 9.7 (500) offline; 9.3 (18) + 9.4 (146) + 9.6 live (10) live-LLM.
...
- **No streaming progress for the LLM call itself** — the user sees "AI is analyzing the document…" for tens of seconds at a time. Streaming the partial LLM output for user reassurance is still a polish item left for a future release. v3.0.17 mitigated the WORST symptom of this at the UI level — a per-step elapsed-time counter and an honest "large documents can take a minute or more per phase" note in the frontend (`phaseStartedAt`/`elapsedTimerId` clock writing `#ing-elapsed` in [src/public/next/views/ingest.js](../src/public/next/views/ingest.js) — it was `showProgress`/`tickProgressElapsed` in the `src/public/app.js` deleted in v3.41.0), so a stalled-looking Phase 1 reads as "still working" instead of "hung" — but the underlying call itself is still one opaque request with no sub-progress to report.
...
Test coverage: **77 offline assertions** in [`scripts/test-chat-intent.js`](../scripts/test-chat-intent.js) (the two reported questions, the full decision/enumerate/synthesis matrix, every audit regression class, `extractAsk` focus extraction, and the echo stripper incl. comma-list / code-block / multi-path-citation preservation) + **33 offline** in [`scripts/test-chat-truncation.js`](../scripts/test-chat-truncation.js). Live: **16 assertions on real Gemini AND Anthropic** in [`scripts/test-chat-intent-live.js`](../scripts/test-chat-intent-live.js) (decision + analytical answers on the large `articles` domain are focused <9k chars, no catalogue echo) + **28** in [`scripts/test-chat-truncation-live.js`](../scripts/test-chat-truncation-live.js).
...
- Every cap (12288 / 16384 / 20480) is within the output limits of every model chat can run on (Gemini 65536; Anthropic Haiku clamped to 64000 in `llm.js`; the lowest ceiling among the offerable OpenRouter models is 117,900), and chat is **text mode** — on overflow the §10c/v3.0.7 path returns a partial answer with a note rather than failing, so a larger cap is always safe.
...
**Rendering (nicer output).** Chat answers render Markdown → styled HTML via [`src/public/next/shared/markdown.js`](../src/public/next/shared/markdown.js) — the module that replaced `src/public/markdown.js` when `app.js` was deleted in v3.41.0. It exports `renderMarkdown` as an ES module (not the old `window.renderChatMarkdown` global) and now serves two surfaces: the assistant's chat bubbles and the wiki reader overlay. It is **XSS-safe by construction**: it HTML-escapes the whole string FIRST, then inserts only a fixed allow-list of tags (`<strong>`, `<em>`, `<code>`, `<pre>`, `<ul>/<ol>/<li>`, `<p>`, `<div class="md-h">`, and styled `<span>`s for `[source:]` chips + `[[wikilinks]]`) — no user text is ever placed in an attribute or URL, and there is no `href`/`src` sink. Inline code is isolated by a **structural split** (not a text placeholder) so answer text can't forge a sentinel. An independent audit confirmed the XSS surface is closed.

Test coverage: **66 offline assertions** in [`scripts/test-chat-style.js`](../scripts/test-chat-style.js) (normalisation incl. prototype-key inputs, cap/directive table, 4-arg≡balanced no-regression, orthogonality to intent, comprehensive-keeps-anti-dump-rules, route + `sendMessage` wiring) + **222 offline** in [`scripts/test-next-markdown.js`](../scripts/test-next-markdown.js) (successor to `test-chat-markdown.js`, deleted with `app.js` in v3.41.0; XSS vectors, block/inline formatting, citation chips + wikilinks, code-sentinel-forgery guard, defensive inputs — against `/next`'s renderer, the only Markdown renderer left) + **17 live on real Gemini AND Anthropic** in [`scripts/test-chat-style-live.js`](../scripts/test-chat-style-live.js) (the monotonic length ordering, all focused, no echo, garbage→balanced, and the comprehensive+enumerate no-dump guard).

## 10e. Per-chat model selection (chat lane only)

**This section describes the READ side, and the single most important thing about it is that it cannot touch the write side.** The chat composer lets you choose the exact model that answers one message. Ingest, Wiki Health and Compile always run on the **active provider's** model and never read that choice. The two lanes are separated structurally rather than by convention: `generateText` carries provider and model in a trailing options object, and every Health call site and `compile.js` pass fewer arguments than that — an override there is not merely unused, it is inexpressible. `ingest.js` is the one exception, because it does pass an options object (for usage reporting and the abort signal), so an override is syntactically expressible there and is simply never passed; ingest's protection is the route-level concurrency guard instead.

**What the composer chooses.** Originally (v3.0.11) the control picked a *provider*, and the model id was always that provider's pinned default. Since v3.13.0 it picks an **exact model**, because the price shown beside a row has to be the price actually billed. The menu is grouped by provider, ordered cheapest first, and lists what the key(s) saved in Settings actually make available. One key is enough: the models in `OFFERABLE_MODELS` ([llm.js](../src/brain/llm.js)) are all **hand-measured**, spread across the three providers — every one probed against this repo's real ingest prompt before being offered — and an OpenRouter key can fetch many more for chat (see below). The table is the count; this doc deliberately does not carry a copy of it. With no key at all there is nothing to choose and the control is not rendered.

A much larger **chat-only** catalogue is admitted from OpenRouter's live API at runtime, wired through the same admission factory so a dynamically-admitted entry carries its own price. It is populated on demand by `POST /api/config/openrouter/sync` (Settings → *Refresh model list*), never automatically: until a user refreshes, an OpenRouter key offers the three measured models above; afterwards it offers those plus every catalogue entry that passed the structural filters — 189 on one measured run. Every fetched entry is **chat-only by construction** and can never build a wiki. See [model-lifecycle.md → OpenRouter](model-lifecycle.md#openrouter--a-third-provider-whose-catalogue-moves-without-us).

**Plumbing.** The client sends an optional provider and an optional model. `sendMessage` (`src/brain/chat.js`) runs them through two sibling validators before anything reaches `generateText`:

- `normalizeChatModel(provider, model)` — two independent gates, **both** required: the model must be on the allow-list for that provider, **and** that provider must have a key saved in Settings (`getApiKeys()`, never `getEffectiveKey`). The allow-list lookup is a `Map.get` on a memoised id index (v3.57.0; an array scan with `===` before that), and a `Map` has no prototype chain, so `__proto__` / `constructor` / `toString` cannot resolve to anything; this wrapper deliberately adds no object lookup of its own so that property is inherited rather than remembered.
- `normalizeChatProvider(provider, normalizedModel)` — the same saved-key rule, plus: if a model was honoured, the provider offering it is honoured with it; otherwise the provider must be able to resolve its own default model. That coupling is the point, not a convenience — the two values are passed to `generateText` independently, so a provider refused while its model was accepted would resolve the *global* active provider and then discard the model as not offerable there, silently answering on and billing a provider the user did not pick.

The hand-off between the two is **self-validating**: the model is re-checked against the allow-list inside the provider validator, so a caller passing arbitrary truthy junk cannot use it to skip the default-model gate. Anything invalid falls back rather than throwing — to the provider's default model, and then to the global active provider — and since a provider's default is its cheapest measured model, a refusal can only ever spend *less* than was asked for.

**The default-model gate is derived, never a hardcoded provider list.** It asks `llm.js` the only question that matters — does a default model actually resolve for this provider — so a provider becomes selectable the moment it genuinely can serve one, and a fourth provider needs no edit at that site. It also fails safe on a missing export: `llm.js` is imported as a namespace and the function is type-checked before being called, so a renamed or not-yet-shipped export degrades to "this provider cannot serve a model-less turn" rather than a TypeError at the trust boundary.

**Availability is scoped to saved Settings keys, not `.env`.** This was corrected after a real bug: an earlier version keyed availability off config-**or**-`.env`, so a provider Disconnected in Settings but still present in `.env` stayed in the menu **and** was still callable. Now both the menu and the backend gates mirror the saved-keys state exactly, and a stale selection persisted in the browser falls back rather than resurrecting a disconnected provider. The `.env` fallback still drives the *global* active provider for the documented developer use; only the per-chat selection is config-scoped.

**The answer records what actually served it.** `sendMessage` returns — and the conversation JSON persists — the provider and model **the provider itself reported**, not the one requested, together with the four token counts for that turn. Reporting the request instead of the outcome passes every refusal test and fails only the fallback walk, so a silently-refused model would otherwise still be displayed as the one that answered. The counts are persisted rather than a computed dollar amount, so a reloaded thread and a live thread render the cost from one input; a message with no recorded model renders a neutral provider label, never the current selection. Both fields are written only when the provider actually reported them, so existing conversations load unchanged and a missing value means *unknown*, never *the default*.

**Known and deliberate:** a request carrying a model but **no** provider resolves to the provider default, even where the model would have been valid for the global active provider — the model gate keys on the provider the caller named. The shipping client never produces that shape (it always sends the two together), and refusing is the safe direction on both money and correctness, so this is not widened speculatively.

Test coverage: `scripts/test-chat-model.js` (offline — the validators against every invalid input including prototype keys, key-backed honouring, the served-model record, and end-to-end source guards), plus the composer and picker suites under `scripts/test-next-*.js`.
...
Test coverage: **228 offline** assertions in [`scripts/test-next-chat-compile.js`](../scripts/test-next-chat-compile.js) — successor to `test-chat-compile-card.js` (36 offline source guards), deleted with `app.js` in v3.41.0. It covers the same property against `/next`'s `views/chat.js`: the outcome card is appended to the thread rather than un-hiding a fixed panel, `renderCompileOutcome`/`pushCompileCard` is the single entry point, the navigated-away (conversation/domain changed) guard, the card rule carries no `max-height`/`overflow`/`flex-shrink`, and the composer/thread layout is undisturbed — plus the Domains→Chat scope-handoff API the old suite never had to cover.
...
Test coverage: **`scripts/test-ingest-queue.js`** (**352 offline assertions**) covers the backend queue/worker below; it grew substantially across the two adversarial audit rounds that found and closed the concurrency bug (§10g.1), the stranded-item invariant (§10g.1b), and the path-scrubbing/allow-list rewrite (§10g.3b). Its frontend counterpart, `test-ingest-queue-frontend.js` (225 offline assertions against `src/public/app.js`), was deleted alongside `app.js` in v3.41.0; `/next`'s own ingest-view frontend — the destination sidebar, drop zone, budget input and queue-mode gating — is now covered by **`scripts/test-next-ingest-view.js`** (**484 offline assertions**), which is not a line-for-line port of the deleted suite's specific H1/H2/M4 audit-fix assertions below (recorded here as history of what those fixes were, not as current coverage), plus **`scripts/test-next-ingest-dropzone.js`** (**158 offline assertions**), which is behavioural rather than source-scanning: it builds a DOM, mounts the real view functions against it, and dispatches `dragenter`/`dragover`/`drop` with a fake `dataTransfer`. All three suites are pure offline; no network, no LLM calls, no cost.

**Why the drop path needed its own suite (v3.46).** Drag-and-drop was inert in the Mac app for the whole of the `/next` shell's life while `test-next-ingest-view.js` §4 stayed green, because every assertion about it was a regex over source text and none of them dispatched an event. The cause was that `dragover` called `render()`, which goes `renderMain` → `setMain` → `#view-root`.innerHTML — so the element the pointer was holding a file over was destroyed by the drag's own first event. The rule the view now holds to is that **a live drag mutates, it never re-renders**: the only thing a drag changes is one class on the zone's root element, the two copy variants both ship in the markup and CSS picks between them, and `setDragActive` is the single writer of `state.dragActive`. The new suite's load-bearing assertion is node identity — the `#ing-drop-zone` object is `===` the same object after six `dragover`s — with a positive control proving a render really does replace it, so the check is capable of failing.
...
**Historical — this paragraph describes the now-deleted `test-ingest-queue-frontend.js` against the now-deleted `app.js`; it is kept as a record of what the H1/H2/M4 audit fixes were, not as current coverage.** The frontend suite covered the original ground (the single-file path is provably unchanged when only one file is selected, cost/estimate formatting never fabricates a number, the shared ingest busy-gate refcount balances across every status transition sequence, no `alert()`/`confirm()` anywhere in the new UI, filenames are HTML-escaped in every rendered row) plus the audit findings: the H1 done-summary accounting (every item lands in a labelled bucket, including a status the server has never sent before, so nothing can silently vanish from the totals — with a specific carve-out verified for a *cancelled* batch's untouched items, which are expected to still read `pending` and must not render with the same alarm styling as a genuinely unaccounted item elsewhere), `job.failReason` actually rendering in the done summary, the H2 busy-gate domain-key pairing (the key recorded at ENTER time is what's used to release at EXIT, never a value re-read from the domain dropdown at some later moment, which is what let a resumed batch on a page reload silently leak the busy gate open forever), the M4 fix making the visible custom dropdown control actually honour a disabled underlying `<select>` (it previously stayed fully clickable while `renderQueuePanel` believed it had locked the domain picker mid-batch), bidi-control-character neutralisation in displayed filenames (a display-integrity fix, not an XSS one — every user string here already goes through `escHtml`), and the same-domain collision guard described in §10g.7's frontend paragraph, verified in both directions (refuses on the same domain the batch is running against; permits a different domain) with a RED-confirmed regression check that the guard being bypassed really does leave the Ingest button clickable.

### 10g.9b — How a drop reaches the queue (v3.46)

A dropped file and a picked file go through **the same** `handleSelectedFiles`, so every rule in §10g.3 and above applies identically to both: one file with no batch in progress stays on the single-file path, two or more start a batch, and a later drop *accumulates* onto the selection rather than replacing it. There is no drop-specific branch anywhere in the selection logic, which is why "drop three files" and "pick three files" cannot diverge.

Two view-level guards sit around it, both in `src/public/next/views/ingest.js`:

- **A drop anywhere in the full-page Ingest view is routed to the zone; in section mode the same listener is scoped to the host element.** A document-level `drop` listener (installed once per mount in `onEnter`, removed in the teardown — never in `wireListeners`, which runs on every render) forwards a file drop that missed the zone. Since v3.64.0 its *convenience* half checks `dropLandedInHost(e.target)` when the panel is hosted, so a file dropped elsewhere on the domain page is not forwarded; its *refusal* half — the `preventDefault` below — is unscoped and still catches every drop. A drop *inside* the zone is deliberately ignored by that listener, because the zone's own handler has already taken it on the way up; handling it twice would queue every file twice.
- **A file drop is always `preventDefault`-ed, even when nothing acts on it.** The browser's default action for a file dropped on a page is to navigate to it. In a tab that shows the PDF; in the Electron shell it takes the window off the app with no address bar and no visible way back. `desktop/main.js` carries the second half of that guard — a `will-navigate` / `will-frame-navigate` listener that refuses any navigation away from the app's own loopback origin (same-origin is allowed through, because Settings' post-update `location.reload()` depends on it).

While a batch job is on screen there is **no drop zone**, because files cannot be added to a job that has started (§10g.9's one-at-a-time rule is what makes the selection final at Start). A drop onto that panel is therefore not acted on — but it is not swallowed silently either: the panel says the files were not added, and names the recovery (wait for the batch, or **Dismiss** a finished one and drop again).
