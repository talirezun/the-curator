...
**What the composer chooses.** Originally (v3.0.11) the control picked a *provider*, and the model id was always that provider's pinned default. Since v3.13.0 it picks an **exact model**, because the price shown beside a row has to be the price actually billed. The menu is grouped by provider, ordered cheapest first, and lists what the key(s) saved in Settings actually make available. One key is enough: **seventeen** models are on offer in this release — seven Gemini, seven Anthropic, three OpenRouter — and every one of them was hand-measured against this repo's real ingest prompt before being offered. With no key at all there is nothing to choose and the control is not rendered.

There is a mechanism for admitting a much larger chat-only catalogue from OpenRouter's live API at runtime, and it is wired through the same admission factory so a dynamically-admitted entry carries its own price. **Nothing populates it in this release**, so what an OpenRouter key actually offers today is the three measured models above, not the provider's full catalogue.
