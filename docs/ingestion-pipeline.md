...
- `normalizeChatModel(provider, model)` — two independent gates, **both** required: the model must be on the allow-list for that provider, **and** that provider must have a key saved in Settings (`getApiKeys()`, never `getEffectiveKey`). The allow-list lookup is an array scan comparing with `===`, so `__proto__` / `constructor` / `toString` cannot resolve to anything; this wrapper deliberately adds no object lookup of its own so that property is inherited rather than remembered.
