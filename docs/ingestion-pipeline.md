...
- **A drop anywhere in the Ingest view is routed to the zone.** A document-level `drop` listener (installed once per mount in `onEnter`, removed in the teardown — never in `wireListeners`, which runs on every render) forwards a file drop that missed the zone. A drop *inside* the zone is deliberately ignored by that listener, because the zone's own handler has already taken it on the way up; handling it twice would queue every file twice.
