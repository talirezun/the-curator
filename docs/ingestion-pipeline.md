...
**What the composer chooses.** Originally (v3.0.11) the control picked a *provider*, and the model id was always that provider's pinned default. Since v3.13.0 it picks an **exact model**, because the price shown beside a row has to be the price actually billed. The menu is grouped by provider, ordered cheapest first, and lists what the key(s) saved in Settings actually make available. One key is enough: **seventeen** models are hand-measured — seven Gemini, seven Anthropic, three OpenRouter — every one of them probed against this repo's real ingest prompt before being offered, and an OpenRouter key can fetch many more for chat (see below). With no key at all there is nothing to choose and the control is not rendered.
