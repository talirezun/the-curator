...
**The menu is narrow by measurement, and the budget is now arithmetic rather than a target.** Measured through the READER's view on the maintainer's real store — `CURATOR_TEST_USER_DATA_DIR` at the installed app's Application Support folder, which is the view that matters and the one every suite used to miss — the widest rendered line is **41 characters** and the widest item about **259 points against a 260-point target**. Before this work that same view measured **74 characters and about 517 points**, twice the budget. The figures come from one formula rather than from eye: `labelBudgetChars(iconPoints, charPoints)` spends `MENU_WIDTH_POINTS` (260) minus `MENU_CHROME_POINTS` (36) minus the icon and its `MENU_ICON_GAP_POINTS` (4) bearing, at `MENU_CHAR_POINTS` (6.5) per glyph — which is why a row carrying an 11-point recency dot gets **32** characters where a plain label gets **34**, and why a `sublabel`, drawn smaller at 5.0 points per glyph, gets **41**. Every one of those numbers is an assumption about a font nobody has measured except `MENU_WIDTH_POINTS`, so the suite asserts the **arithmetic** and reports the **sensitivity** across the plausible advance range rather than pinning a character count as a fact. Four levers sit on top of the budget, each decided from the data at render time rather than hardcoded: the project token is dropped when only one project has state; a leading `session-` is stripped; a harness shared by every shown local row is dropped; and the machine is dropped when every shown row resolves to one computer. Each comes straight back the moment it distinguishes something — a second project, a scope that does not share the prefix, a second harness — because a hardcoded strip would be a lie waiting for the day the user's setup changes, and a silent one. **The absolute rule on top of all of them: every fact removed from a label is still in that row's tooltip, in full**, which is why the tooltip is built from the raw values.
...
| 2 | `    menubar-widget-design` | no | a statement about the line above; carries the uncompacted `project · scope` as `toolTip` whenever compaction changed it |
| – | **header** `Save pulse` | no | `type: 'header'` |
| 2b | `4 days known · 70 saves` | **yes** | the strip in the icon gutter, the reading on the label |
| – | **header** `Recent scopes` | no | `type: 'header'` |
| 3 | up to **five** rows | yes | newest first, flat, each with a recency dot in its icon gutter |
| 3b | `More in Agent Memory… (6)` | **yes** | the overflow, and a real destination |
| – | separator | | **only when there are notices** |
| 4 | notices | no | at most `MAX_NOTICES` (4), then `…and N more` |
| – | separator | | always |
...
|---|---|---|---|---|
| `live` | under **2 min** (`LIVE_WINDOW_SECONDS` 120) | **green** | filled disc, r 3.0pt | 28.3pt² |
| `warm` | 2 min – **30 min** | **green** | ring, r 3.0pt, stroke 1.2 | 18.1 |
| `today` | 30 min – **12 h** | **amber** | ring, r 2.5pt, stroke 1.1 | 13.5 |
| `cool` | 12 h – **7 days** | **grey** | ring, r 2.0pt, stroke 1.0 | 9.4 |
| `cold` | over 7 days | **grey** | filled dot, r 1.2pt | 4.5 |
...
**Colour is never the only signal.** Green and amber are the classic confusion pair for the commonest colour vision deficiencies, and their luminances cannot be pushed apart without one approaching the contrast floor — so the five states are also a **strictly decreasing ladder in ink**, asserted by the suite from the shipped geometry rather than from a comment. `live` is the only filled disc at full size, so the one state that changes what a person does next is separable with the colour removed entirely. `cold` returns to a filled shape at **16%** of `live`'s area, which no viewer will confuse with it. (The source comment in `menu-dots.js` says 19%; the shipped geometry gives 4.5 / 28.3 = 16.0%, and `dotInkArea()` — which the suite asserts against — is the authority.) `dotToolTipLine()` additionally states the band in words, because a legend that exists only in a design document is a legend the user does not have.
...
**The canvas is 11pt and odd on purpose.** A circle centred at `size / 2` on an EVEN canvas is centred on a pixel *corner*, so at 1x — where a 1.2pt dot is 2.4 device pixels across — the mark has no fully-covered pixel anywhere and a `cold` dot that should be solid renders as a soft blur. On an odd canvas the centre is the middle of pixel 5, the smallest filled dot has a solid core, and pixel 5 lies entirely inside every ring's hole, so the **disc/ring distinction — the one that does not need colour — survives at 1x**. The first draft was 10pt and the suite caught it.
...
**Three cell states, separated by SHAPE rather than opacity.**

| State | Drawn as | Means |
|---|---|---|
| **active** | a **12pt** full-height bar, on a five-rung colour ramp keyed to the save count and saturating at 5 | saves landed in this cell |
| **empty** | a **3pt** stub on the baseline, grey | this cell existed and nothing happened |
| **unknown** | a **1pt** baseline hairline, grey | before the store existed at all |

The heights are the second signal, the one that survives when colour does not: **12 / 3 / 1 points**, an empty stub a quarter of an active bar and an unknown hairline a third of an empty stub, chosen to be unmistakable rather than tasteful. The ramp is `activeLevel(count) = min(5, count)` — it saturates rather than running away, so one pathological cell cannot redefine what *busy* looks like for the rest of the strip.
...
**The ramp encodes CADENCE and must never encode anything readable as quality or progress.** An agent told to *save early and often* — which is exactly what `claude-skills/curator-continuity` asks for — produces more marks than one told to save twice. Drawing a denser column darker would say *more progress* when it means *different instructions*. The label therefore says "saves", never "activity", and no legend anywhere ranks a dense column above a sparse one.
...
**Geometry, and why these numbers.** 3pt bars at a 4pt pitch, 14pt tall — **55 × 14 points**, emitted as a colour RGBA PNG at 1x and 2x from one drawing (`stripCanvas(cells, scale, palette)`), so a retina asset cannot drift from its low-resolution twin. It was **83 × 11 with 2pt bars at a 3pt pitch**, and every one of those numbers moved in the same change: width **83 → 55pt** (34% narrower), height **11 → 14pt**, bar **2 → 3pt**, which is twice the ink in an active cell. A strip that is more present and smaller at once is the only honest answer to *make it more visible* inside a shrinking width budget. Electron performs **no scaling** on a menu icon, so the image draws at its declared size and **widens the menu** — which is why the strip is the only lever available: a per-row character budget cannot shrink a fixed-width image. **The 28 points came out of the drawn RESOLUTION and not out of the producer's contract.** `PULSE_WINDOW_SECONDS` (7 days) and `PULSE_BUCKET_SECONDS` (6 hours) in `src/brain/tray-summary.js` are untouched and still produce 28 buckets; `mergeCells(cells, mergeFactor(28))` folds them two-to-one at draw time, so one drawn cell is **twelve hours** — morning and evening of each of seven days, which is a timeline a person can read. Seven daily cells were considered and refused on the **ramp** rather than the picture: counts per six hours run about 1–9, which five rungs discriminate, while counts per day run 5–40 and would pin the ramp at saturation permanently, destroying the only reading the colour carries. `drawnBucketSeconds()` is what the tooltip's legend quotes, never `bucketSeconds`, because a legend must describe the picture in front of the reader.

**It is NOT a template image, and the constraint it inherited from the glyph was false.** The tray GLYPH must be a template because macOS tints a menu-bar icon; a MENU ITEM icon is drawn as authored, so the strip and the recency dots ship as full colour with `template: false`, and the consumer must not call `setTemplateImage(true)` on them. That is what makes the contrast measurable at all: the alpha-only version's heaviest bar was alpha 1.0 of the *disabled-text* tint, roughly a quarter of the label colour's opacity, which put it under 2:1 against the menu — the maintainer's *"barely visible"*. Every colour in `STRIP_PALETTE` now clears **3:1** (WCAG 2.2 1.4.11, the non-text floor — 4.5:1 is the text floor and citing it here would cite the wrong rule) against all three backgrounds of its theme's `MENU_BG_BAND`, worst case 3.65 for the lightest active rung and 9.33 for the heaviest. **The lightest ACTIVE rung must outweigh an EMPTY stub**, or a cell holding one save reads as quieter than a cell holding none — the first palette written here failed exactly that (3.15 against empty's 3.29) and the suite caught it, not the eye. **UNKNOWN is deliberately heavier than EMPTY**, carried over from the alpha version for the same reason: making *no data* the faintest thing on the strip would encode it as *even less than nothing*. `scripts/test-tray-paint.js` recomputes every ratio from the shipped values with a control proving the check can fail, so the numbers in the source comment are a convenience and not the guarantee.
...
- **The pulse strip and the recency dots have never been drawn by AppKit.** Their geometry, the strip's three cell states, the dots' five, their labels, their contrast ratios and their actual decoded pixels are executed and asserted offline, and `sips` opens the emitted PNGs to prove validity by something other than their author's encoder — but that Electron accepts a `MenuItemConstructorOptions.icon` of these dimensions, that macOS leaves a **non-template colour** image alone in a light bar, a dark bar and an inverted highlighted row, and that the 2x representation is picked on a retina display are all **unproven**. `main.js`'s wiring of them is source-scanned only, and the suite says so in its own headings.
