...
flowchart LR
    ...
    STR -->|"alpha-only template PNG<br/>1x + 2x"| TPL
...
**The menu is narrow by measurement, and every lever is conditional.** The widest rendered line went **87 → 54 characters, with zero lines over 56** (was 16). Four levers, each decided from the data at render time rather than hardcoded: the project token is dropped when only one project has state; a leading `session-` is stripped; a harness shared by every shown local row is dropped; and `MAX_HEADLINE_CHARS` came down from 72 to 54. Each comes straight back the moment it distinguishes something — a second project, a scope that does not share the prefix, a second harness — because a hardcoded strip would be a lie waiting for the day the user's setup changes, and a silent one. **The absolute rule on top of all of them: every fact removed from a label is still in that row's tooltip, in full**, which is why the tooltip is built from the raw values.
...
One menu item, directly under the headline pair it elaborates, carrying a drawn strip in its icon gutter and the reading in words on its label — `Save pulse · 4 days known · 41 saves`. **28 buckets of 6 hours across a 7-day window, oldest on the left.**
...
|---|---|---|
| **active** | full-height bar, alpha `0.60 + 0.13·(n−1)` capped at 1.0 | saves landed in this bucket |
| **empty** | full-height bar, alpha `0.16` | this bucket existed and nothing happened |
| **unknown** | a 1pt **baseline tick**, alpha `0.30` | before the store existed at all |

Empty and unknown must not render identically — that is this repo's fact-versus-absence rule, and it is not a corner case: on a 3.5-day-old store against the 7-day window **13 of 28 cells are unknown**. Two similar alphas do not read at 2pt of width, so the distinction is carried in shape. Unknown is deliberately *not* fainter than empty; making "no data" the faintest mark would encode it as *even less than nothing*. Every cell is drawn, empties included — a strip that omits them stops being a timeline and becomes a scatter of marks with no scale.

**Alpha encodes CADENCE and must never encode anything readable as quality or progress.** An agent told to *save early and often* — which is exactly what `claude-skills/curator-continuity` asks for — produces more ticks than one told to save twice. Drawing a denser column bigger would say *more progress* when it means *different instructions*. The label therefore says "saves", never "activity", and no legend anywhere ranks a dense column above a sparse one.

**The label states only what the data can support**, and both honesty fields change the label rather than hiding in the tooltip, because a caveat you have to hover to find arrives after you have already believed the picture: `coversWholeWindow: false` names the span actually covered (*"4 days known"*) instead of the window, and `pairsTruncated > 0` makes the count a floor (*"at least 65 saves"*). `clock: 'none'` replaces the sentence entirely rather than qualifying it — *"Save pulse · no save times recorded"* — and a strip whose every cell is unknown is captioned *"nothing recorded yet"*, never *"7 days · no saves"*.

**Geometry, and why these numbers.** 2pt bars at a 3pt pitch, 11pt tall — **83 × 11 points**, emitted as an alpha-only template PNG at 1x and 2x from one drawing (`stripMatrix(cells, scale)`), so a retina asset cannot drift from its low-resolution twin. Electron performs **no scaling** on a menu icon, so the image draws at its declared size and **widens the menu** — which is why the width was checked against the menu's own budget in the same release that was narrowing it, rather than picked by eye. At an assumed 6.5pt average glyph advance the strip costs about 12.8 character-widths; the suite asserts the item is not the widest line in the menu at every advance from 5 to 10pt.

It is a **template image** for the same hard reason as the glyph: macOS takes only the alpha channel and tints it for a light bar, a dark bar and the inverted highlighted row. There is no colour available. `encodeAlphaPng` is imported from `tray-icon.js` rather than reimplemented — a second hand-rolled CRC32/deflate encoder in the same folder is a second thing to get wrong.

The item is **disabled**, which is this menu's existing idiom for a statement rather than an action, and the same idiom the notices and the freshness stamp already use. That costs contrast, which is the right trade here and the wrong one for the headline: the headline is the *answer* and stays enabled at full contrast, and clicking a picture of the last seven days would have no obvious destination. `nativeImage.createFromBuffer` is **injected** as `makeIcon` rather than imported, so every decision — whether there is a strip at all, what it says, where it sits, whether it is actionable — stays inside a module `npm test` executes for real. With no `makeIcon` the item still appears with its label and tooltip and simply carries no picture: a missing image must not cost the reading.

**One strip over the whole store, not one per row.** A per-row sparkline was refused on legibility — eight to eleven independent bands a few points tall, in a menu this same release was making narrower, each drawn from a single scope's handful of saves, most of which would be one mark and a lot of empty.
...
- **The pulse strip has never been drawn by AppKit.** Its geometry, its three cell states, its labels and its actual decoded pixels are executed and asserted offline, and `sips` opens the emitted PNG to prove validity by something other than its author's encoder — but that Electron accepts a `MenuItemConstructorOptions.icon` of these dimensions, that macOS tints an alpha-only template strip correctly in a light bar, a dark bar and an inverted highlighted row, and that the 2x representation is picked on a retina display are all **unproven**. `main.js`'s wiring of it is source-scanned only, and the suite says so in its own headings.
