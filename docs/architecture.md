...
**Chat's streaming turn is the one case where a stale mount's work is re-adopted rather than discarded (v3.64.1).** The fetch behind a chat turn was never tied to the mount token — leaving Chat mid-answer and coming back was always going to find the request still running server-side. What v3.64.1 fixed was the *render*: `onEnter` used to blank `state.sending`/`state.sendStream` on every entry and the mount token always moves on a `navigate()`, so a still-running turn's record could never match a freshly-mounted view again — the answer only reappeared once `selectConversation` re-read the finished conversation off disk. `sendAbort` (the module-level handle a send creates) now carries the stream's accumulated buffers and its start time, not just a way to cancel it, and a new `adoptLiveTurn(token)` re-points that live handle at the *current* mount: it proves the domain and the conversation still match before doing anything, re-hooks the delta buffers so further `consumeChatStream` output paints into the newly-mounted DOM, restores the UI's sending lock, re-arms the elapsed-time clock at the turn's original start (so the clock reads real elapsed time, not zero), and pushes the original question back into the thread. `selectConversation` calls it on entry; `consumeChatStream` takes a token callback specifically so a delta that arrives *after* re-adoption still finds the right mount. This is a deliberate, narrow exception to the token discipline above, not a relaxation of it — the token is still what proves *which* mount a result belongs to; `adoptLiveTurn` uses that same proof before it will re-attach anything.
...
per fact, and both pairs work the same way: the producer sets a **module variable**, the consumer
reads it **once and clears it**, so a stale request can never resurface on a later, unrelated visit.
...
| **`requestDomainFold(foldId)` → `consumeDomainFoldRequest()`** (v3.64.0) | First-run guidance → `views/domains.js` | *Open Domains* lands on the **Ingest** section. Through v3.65.1 the section was a fold and this request **opened** it; **since v3.65.2 Ingest is no longer a fold** (see below), so the request instead scrolls its head into view and focuses its title (`landOnSection`), and writes no preference. `ADD_SOURCES_FOLD` and `requestDomainFold` keep their v3.64.0 names although they now land rather than open — both are pinned by name in two suites |
| **`requestSettingsSection(id)` → `consumeSettingsSection()`** (v3.65.2) | Project context → `views/settings.js`, consumed in `onEnter` before the first paint | The Documents step's GitHub panel, with no read-only token saved, offers a door — *"Add one in Settings"* — that calls `requestSettingsSection('storage')` and then `navigate('settings')`. Settings opens directly on the **Knowledge base** section instead of whatever section it last had; an unknown id falls back to the default section. Built on the exact shape of `requestDomainFold`/`consumeDomainFoldRequest`: trimmed, a falsy value clears it, and consuming it self-clears |
...
**`src/public/next/shared/chat-scope.js`** in v3.62.0, so the two views that now offer *Ask this
domain* call one function rather than each carrying a copy of the hazard.
...
**The domain page's fold and lens preferences moved from per-domain to install-wide storage in v3.64.1.** `localStorage['curator-domain-sections-v1']` held, through v3.64.0, one entry per domain slug; a value is now read and validated as either that legacy per-domain shape (migrated in place on first read) or the new shape, a single `{"*": {...}}` entry that applies to every domain on this browser — whether INGEST and SHARED BRAIN are open, and which of Wiki/Context/All the PAGES lens shows. An explicit close writes `{"*":{"sources":false}}` once, and every domain page reads that one key from then on; a domain with no ingest yet still *derives* INGEST open (nothing is written until the user acts), so the install-wide default never hides the one section a brand-new domain needs. Two related defects found by looking during that work, both now guarded: `<details open>` created via `innerHTML` fires a `toggle` event, so every paint of an already-open fold was reaching the preference writer — harmless while the written value already matched, but a real hazard the moment a fold could be opened by anything other than the user's own click; and comparing an idle hosted-section shell against a freshly rendered one for the no-repaint check could never find them byte-equal (the hosted panel owns its own body), so the comparison used to force a full remount of both panels on every paint — the compare now covers only the `<summary>` the page itself owns, and the panel's body is left alone.

**Ingest is no longer a fold at all, as of v3.65.2** (`I1`; the maintainer: *"Ingest is number one but hidden below a drop-down — it's important, not long, it should be exposed. Get rid of it."*). Section ① is now a plain `<section id="dm-sources-fold">` with no `<summary>` and no `open` attribute — the id is unchanged, so `patchMainAroundHosts` still recognises the hosted panel by it and the D-J held-drag guarantee holds exactly as before. The row's reading ("last ingest N ago") moved into the section's own head row, right-aligned, the same face every other section's reading uses. `readSectionPrefs` no longer reads the `sources` key at all — an older stored value is silently dropped on read and falls out of the file on the next write, with no migration write of its own (a read still never writes). Since Ingest is not a fold, the patch's open-state carry (which used to copy a `<details>`'s `open` across a repaint) is now gated on `before.tagName === 'DETAILS'`: without that guard, a plain `<section>`'s `.open` reads `undefined`, which is never `=== true`, so the carry logic was pushing a phantom `'sources'` entry onto `programmaticFolds` and setting a stray expando on the section node on every idle paint — closed by the same v3.65.2 commit.
...
- **First-run guidance** (`views/onboarding.js`, v3.8.0; two doors since v3.61.0) — real, and deliberately **not** a port of the pre-redesign frontend's blocking 4-step modal (that frontend, and its modal, were deleted in v3.41.0). It is a non-blocking, dismissible panel: no scrim, no `role="dialog"`, no focus trap, and it never steals focus on the automatic path. The `next/` shell's `app.js`'s `boot()` calls `maybeShowOnboarding()` (not awaited, and wrapped in `try/catch`, because `boot()` returning is what sets the `window.__curatorBooted` sentinel the `<head>` guard treats as proof of a healthy load). Every step **points** at the surface that already owns the job, and embeds no input and `POST`s nothing; the domain step navigates to Domains rather than adding a second `POST /api/domains` call site, which `test-next-chat-compile.js` pins at exactly one for the whole tree. **Since v3.61.0 there are two ordered step sets, not one** (`STEP_SETS` = `{knowledge, agent}`), because the original API-key-first order was actively wrong for an agent-harness user the memory layer and MCP bridge do not need a key for: *Build a second brain* still walks `api-key → domain → ingest`, while *Give your agents memory* walks `domain → project → bridge → api-key` with the key marked optional. Neither set is stored as a persona — `deriveDoor()` re-derives which one applies from on-disk facts on every paint, and the doors are shown only while neither fact yet discriminates; pressing a door writes the shell's pre-existing `curator-next-view` last-view key, not a new one. Dismissal is `localStorage` (`curator-next-onboarding-dismissed-v1`) and **fails safe by showing** — a storage throw re-shows the guidance rather than silently hiding first-run setup — which is the opposite direction from the AI-disclosure consent gate, on purpose. It is re-findable from Settings ("Show setup guide"). It is **not** a registered view: like `views/mcp-wizard.js` and `views/shared-brain-wizard.js`, it calls no `registerView()` and owns no rail slot.
- **Project context** (`views/memory.js`; rail caption **Context**, and called *Agent memory* through v3.61.1 — v3.62.0 renamed the presentation only, leaving the view id `memory`, the `mem-*` classes, the filename and `/api/memory` exactly as they were; **v3.65.1 renamed the presentation a second time** — Foundations/Working state/Work-streams/Recent saves → Documents/Memory/Handoffs/Journal, UI copy and docs only, no store name or field moved) — **a real view over the working-state store, since v3.17.0; read-only over tiers 2 and 3, with one tier-1 write since v3.48.0, plus tier-0 writes since v3.59.0.** The store (`src/brain/working-state.js`) is written by agents through the MCP's `get_working_state` / `save_working_state` tools; the view reads it over `GET /api/memory` and `GET /api/memory/:domain/:project` (`src/routes/memory.js`, mounted in `src/server.js`), groups the rail by domain and then by project, remembers the last project per domain, and — since v3.62.0 — renders **four numbered steps** (a fourth, **Session start**, since v3.67.0), since v3.65.0 under the same `renderOverview()` card the Domains page uses (five tiles since v3.70.0 — DOCUMENTS, MEMORY, KNOWLEDGE, AGENT CONNECTIONS (**AGENT SESSIONS** through v3.73.x, renamed app-wide in v3.74.0 — the jump id, route and on-disk fields keep the old word) and SESSION START, the last in tokens, no bar of its own — at a `minTrack` of 253px), with the ⓘ beside each numbered title and no lede under it: ① **Documents** (the project's canonical documents, each with its own source — **since v3.68.0 the head row above the table always shows two doors, *Add from this computer* and *Add from GitHub*, at zero documents and at any number; both doors are always enabled, since v3.69.0 retired the project-wide ownership gate, and a sources strip offers per-source and all-source Refresh**; `src/public/next/shared/foundations-add.js`'s `doorsFor()` is the DOM-free rule for what each door does — `copy`/`mirror` for the local door (Keep in sync / Copy once, defaulting to sync inside a git checkout), `add` for GitHub, given the project's existing sources, never disabled by ownership), ② **Memory** (four closed `.mem-fold` rows — *Agent connections* (*Agent sessions* through v3.73.x, *Capture* through v3.69.0; on-disk name unchanged), *Handoffs*, *The brief*, *Journal* — with every qualifying warning unfolded outside them; the **Last saved** row that used to lead them is gone as of v3.65.1, its fact folded into the MEMORY tile and the Handoffs row's own summary), ③ **Knowledge** (one row per domain the project draws on — the project's own domain an explicit row from v3.65.1, each carrying that domain's identity dot — opening a monitor of five figures from `GET /api/domains/:domain/stats`, with entity/concept/summary each carrying a depth bar against that domain's page count since v3.65.1, plus *Open in Domains*, *Ask this domain* and its own *Remove*, with a **+ Add a domain** picker in a head row above the rows since v3.65.1 [it sat under the last row through v3.65.0]; the chosen set is `knowledgeDomains` on `GET /api/memory/:domain/:project` and is written by `PATCH /api/memory/:domain/:project/knowledge/domains`). Step ①'s sources strip `POST`s `…/foundations/refresh {group}` (one source) or `{}` (all sources, one lock, a failed source named and left untouched, the manifest written once); **its two doors commit through `buildAddCommit()`** — *Add from this computer* `POST`s `…/foundations/add-local {root, files, mode:'copy'|'mirror'}` (`addFoundationsFromFolder`; `mode` defaults to `mirror` when the folder is already inside an existing folder source, else `copy`); *Add from GitHub* `POST`s the v3.69.0 `…/foundations/add-remote {remote, ref?, tokenSource?, files}` (`addFoundationsFromRemote`), joining an existing GitHub source with the same owner/repo and ref or opening a new one — up to `MAX_SOURCES_PER_PROJECT = 8`. **A copied document carries an optional `copiedFrom` (the source folder's basename only, never a path) in the manifest** — emitted only when present, so an untouched project's `get_project_context` payload stays byte-identical, and cleared by `saveFoundation` the moment the owner edits the document in the app, at which point it reads "written by you" like any other kept document. Step ①'s per-row *read first* control `PATCH`es `…/foundations/:slug` — a manifest-only write, which is why it is allowed on a **mirror** while the `PUT` that changes a document's text is refused per document (`ownership-mismatch`, naming the mirror's source; a new slug is never refused). That route is a legitimate app writer even under the read-only rule: it performs a deterministic **byte copy** driven by comparing sha256 against a file on disk, never composes content, so it is not a second author in the sense the rule below guards against. **The read-only half is a design decision, not an unfinished write path:** a browser writer into a *handoff* would make the app a second writer and break the single-writer property the whole sync-safety argument rests on, and it would stamp a human edit with the last agent's provenance. The **standing brief** was never inside that argument — no machine segment, the human's document — so v3.48.0 gives it an Edit that `PATCH`es tier 1, stamped `authoredBy.kind: 'human'`. The Domains view carries the other half of the same write surface: a **Projects** section that creates, renames and deletes projects. What is still not built is the rollup UI (Done/Decided/Blocked composed across scopes) that a pre-v3.17.0 version of this line described — nothing composes across scopes or projects. The three-brains shape — your brain → your team's brain → your agents' brain — is now backed by something on all three. (It stopped being the RAIL's shape in v3.64.0, when Shared Brain moved onto the domain page; the rail is Chat · Domains · Context.) See [working-state.md](working-state.md), [api-reference.md](api-reference.md) and the store's own section below.
...
│   │   │                       documents under state/<project>/foundations/ — v3.59.0; ownership
│   │   │                       init + human PUT/DELETE + a widened repo-scan — v3.61.0; scan
│   │   │                       candidates gain modifiedAt — v3.61.1; DELETE removes a repo-owned
│   │   │                       entry too, gated on the manifest existing rather than on ownership
│   │   │                       — v3.61.1; readFirst — v3.62.0; a REMOTE GitHub mirror arm, repo.remote
│   │   │                       — v3.63.0; knowledgeDomains in project.json, cap 12 — v3.65.0;
│   │   │                       v3.69.0 retires ONE SOURCE PER PROJECT — a document's source is now
│   │   │                       checked per document (source.kind), not per project; manifest
│   │   │                       VERSION 2 adds sources[] (up to 8 groups) only when a manifest
│   │   │                       cannot express itself as v1 — serialiseManifest() always writes the
│   │   │                       LOWEST version that can; an older (≤v3.68.0) app cannot read a v2
│   │   │                       manifest, and a v3.68.1+ app reads it read-only as "newer version").
│   │   │                       Never renders a prompt, never calls an LLM. THREE surfaces WRITE it:
│   │   │                       mcp/tools/working-state.js (read + write), src/cli/save.js (the
│   │   │                       second local client — tiers 2/3, agent provenance) and routes/memory.js
│   │   │                       (reads all four tiers, writes tier 1 AND any kept (written/copied)
│   │   │                       tier-0 document — both stamped authoredBy.kind: 'human'. A MIRRORED
│   │   │                       document's CONTENT is never written here, only mirrored — PUT
│   │   │                       refuses per document (mirrored-slug save refused, new-slug save
│   │   │                       always allowed) — but DELETE may remove a mirrored document's
│   │   │                       manifest entry and copy since v3.61.1, which is a decision to stop
│   │   │                       mirroring it, not a claim about its content, and removes its source
│   │   │                       group too when it was that source's last document; a source
│   │   │                       refresh stays the one legitimate CONTENT write outside a kept
│   │   │                       document's own two — see below). Five more surfaces READ it and write nothing:
│   │   │                       src/cli/context.js, src/brain/chat.js, src/brain/tray-summary.js,
│   │   │                       src/routes/wiki.js and src/routes/chat.js.
│   │   ├── foundation-skeletons.js  FOUNDATION_SKELETONS — the four seed documents an empty
│   │   │                       curator-owned project gets (architecture / decisions / conventions
│   │   │                       / roadmap), plus SKELETON_BANNER (v3.61.0). ONE source read by both
│   │   │                       the create-project route and the Foundations-block init route —
│   │   │                       this repository's most-repeated defect is a template copied per
...
│   │   │                       Chat stays SINGLE-DOMAIN: v3.65.0's `knowledgeDomains` are
│   │   │                       DISCLOSED by views/chat.js (the project group's footer names them,
│   │   │                       and the scope pills they cover are marked) and never SELECTED —
│   │   │                       one lit chip still means the one wiki the answer reads
...
│           │                    key→value lines plus always-visible "loud" warning lines. Eleven call
│           │                    sites in v3.65.0: the MCP bridge connection status, its stale-bridge
│           │                    warning (a `loud` entry on that same monitor), the two self-test
│           │                    results and the session readings; the Sync view's status; Domains'
│           │                    Wiki-health scan report; and, on Context, step ②'s "Last saved" body,
│           │                    its save warnings, the Agent connections reading and the journal's count line,
│           │                    plus step ③'s per-wiki figures (v3.65.0)
...
| A menu bar icon over project context | `lib/tray-model.js`, `lib/tray-menu.js`, `lib/tray-icon.js`, `lib/state-watch.js`, `lib/background-mode.js`, over `src/brain/tray-summary.js` | **Off by default** (`backgroundMode: 'window'`). See [§ The menu bar widget](#the-menu-bar-widget-desktoplibtray-js--srcbraintray-summaryjs). |
...
**`getTraySummary()` is a projection, not a second inventory.** It calls `listWorkingScopes()`, the same function `GET /api/memory` and the MCP's `get_working_state` call, so the tray and the Project-context view cannot disagree about what is on disk. It is its own module rather than part of `working-state.js` for three reasons stated in its docblock, of which the load-bearing one is import-graph hygiene: `remote` is a Personal Sync fact, and dragging `sync.js` into a module the MCP child loads on every spawn would put a `git fetch` inside the stdout-discipline graph.

**The shell asks for MORE rows than it can display, and since v3.48.0 that is deliberate.** The producer's default (`TRAY_DEFAULT_LIMIT`, 8) is what a caller gets when it expresses no opinion; `desktop/main.js` expresses one — `const TRAY_ROW_LIMIT = TRAY_FETCH_ROWS`, imported from `tray-model.js`, which is **40**. It used to be `MAX_ROWS`, exactly what the menu displays, and grouping by project broke that: the five newest PAIRS on disk can all belong to one project, and a model handed only those five cannot group what it was never given — the second project would be invisible while its state sat on disk. The fetch is the data layer's own ceiling and the DISPLAY cap stays where it was: `buildTrayModel()` caps at `MAX_ROWS` (5) in total and `MAX_ROWS_PER_GROUP` (2) within a group, independently, so a caller that asked for more cannot widen the menu. **The counts stay honest across both caps** because `total` and `pairsOnDisk` are taken before the slice: `More in Project Context… (6)` is that subtraction rather than a cap being reported as a measurement.
...
> **This subsection, and the row-composition prose below it up to the per-row submenu
> subsection, documents the menu's PRE-v3.74.0 arrangement and is kept as build history rather
> than rewritten in place.** `desktop/lib/tray-menu.js`'s own top-of-file comment is the
> current, authoritative order ("Layout A"): a save-pulse item on top with a **"Saves by tool"**
> submenu (one strip per normalised tool id, `src/brain/harness-names.js`), then an
> **"Active · last 24 h"** header and one row per **(project × harness)**, tool and age on line
> one, model and headline on line two, with the app's own `freshnessTier` dot replacing the
> draining-clock mark this section describes; notices (including one collision notice per
> work-stream, keyed on the normalised tool so spelling drift can no longer draw two lines for
> one collision) moved directly under the Active rows; an **"Idle · N projects"** row and a
> **"Knowledge · N domains"** row each fold what used to be drawn inline into their own
> submenu. Removed outright, not merely regrouped — kept in the app's Context sidebar and
> Memory step ②, never dropped as facts — are the `Working on: <project> · <age>` headline row
> and its `harness · model` line, and the `Documents` / capture-bars material this section does
> not otherwise describe. The measurements below (menu width, character budgets, the photograph
> of 2026-09-02) are unaffected by the restructure and still hold.

`buildTrayMenuTemplate()` emits one flat template and the order is the design (as it stood
before v3.74.0's Layout A — see the note immediately above):

| # | Item | Enabled | Notes |
|---|---|---|---|
| 1 | `Working on: lumina · 12 min ago` | **yes** | the headline answer, and since v3.48.0 it names the most recently saved PROJECT; clicking opens Project context |
...
| – | **project header** `articles / lumina · 12 min ago · claude-code` | no | `type: 'header'`, one per project, newest project first. The domain qualifies the name only when it distinguishes something — the domain holds more than one project, or two chosen groups would otherwise read alike. The generic `Recent scopes` caption survives for the EMPTY state only, where there is no project to name |
| 3 | up to **two** rows per group, **five** in total across at most **three** groups | — | newest scope first within a group, each with a recency mark in its icon gutter and a **four-item submenu**. Two rows per group is a FLOOR rather than a ceiling: rows a group cannot use are handed back, so a single-project store — which is every pre-v3.48.0 install — still fills all five. A submenu parent carries **no `click`**: on macOS a click on it opens the submenu, and a handler beside that fires unpredictably — `Open in The Curator` is the first submenu item instead |
| 3b | `More in Project Context… (6)` | **yes** | the overflow, and a real destination |
...
| – | separator | | always |
| 5 | `Open Project Context…` · `Open The Curator` · `Settings…` | yes | |
...
**A truncation line is never a measurement.** `truncatedNote` counts against `total`, which `getTraySummary()` takes **before** the display slice, so `More in Project Context… (6)` is 11 minus 5 and not a cap reported as a figure — the defect this project recorded of `distinctScopeCount`. The item is **enabled**, because a disabled line naming a place you cannot go from it is worse than no line at all.
...
The table above describes the **effective** key (config *or* `.env`). The **first-run
guidance panel's trigger does not**: `hasApiKey()` in `views/onboarding.js` (the
pre-redesign shell's `checkFirstRun()`, before that shell was deleted in v3.41.0) reads
`hasGeminiKey`/`hasAnthropicKey`, which come from `getApiKeys()` — `.curator-config.json`
only. A `.env`-only install therefore makes working LLM calls *and* still sees the panel
on every load — harmlessly, because it has been non-blocking and dismissible with one
click since v3.8.0, unlike the pre-redesign modal's forced key entry.
...
Selecting two or more files switches the ingest panel — the INGEST section of a domain page, or the full-page Ingest view — to a **server-owned, disk-persisted queue** (`src/brain/ingest-queue.js`, routed at `POST /api/ingest-queue` and friends) instead of the single-file flow above. It exists so a large batch survives a closed browser tab or an app restart, and so duplicate/partial-failure handling doesn't have to be reasoned about per click.
...
Read tools (v2.3.0+ through v3.59.0, 14 tools)
      ...
      └─ get_working_state and list_projects (v3.17.0 / v3.48.0) are the
           others: they read state/, not
...
Write tools (v2.5.2+ through v3.59.0, 10 tools)
      ...
      ├─ save_working_state (v3.17.0) and save_project_brief (v3.48.0) do
      │     NOT use resolveDomainArg or writePage: they wrap
      │     src/brain/working-state.js, which runs its own
      │     checkProjectWritable (must be a real domain, must not be a
      │     read-only shared-* mirror) and its own resolveInsideState
      │     containment. They write state/, never wiki/. A project name is
      │     resolved by resolveProject, which refuses an unknown or
      │     ambiguous name with candidates rather than choosing one.
      │
      └─ compile_to_wiki:
...
Default domain (v2.5.2+):
      ...
      Set/cleared via /api/config/default-domain (the dropdown in Settings → MCP bridge).
...
| `DESKTOP_HOOKS` | Frozen `['pickFolder', 'relaunch', 'prepareUpdate', 'installUpdate']`. |
...
| `githubTokenKind(value)` | **v3.65.2.** `'fine-grained' \| 'classic' \| null`, from the value's PREFIX alone (`github_pat_…` or `ghp_…`) — never a secret, since the prefix says nothing else about the token |
| `getGithubReadTokenStatus()` | **v3.65.2.** `{ present, last4, kind }` — never the value. `present` uses the same 20-character-minimum test `readGitHubReadToken` applies before it will read with a token, so this status can never say "saved" about a value the reader would refuse |
| `setGithubReadToken(token)` / `clearGithubReadToken()` | **v3.65.2.** The writer and remover for `githubReadToken` — the field the Documents "Mirror from GitHub" panel has pointed at since v3.63.0 and nothing wrote before this release. Validates shape (trimmed; `github_pat_…` or `ghp_…`; bounded length) and throws `GithubReadTokenError('invalid_token', …)` on a bad one; writes through the same `0600` atomic writer the API keys use. Refuses (`config_unreadable`) rather than writing when `.curator-config.json` exists but does not parse as JSON — the older setters in this file still fall back to `{}` on that condition and would silently erase every other key, which this writer will not risk for a credential file. The field name is **typed here**, not imported from `github-read-client.js`, because this module sits on the static import graph `test-tray-summary.js` walks to prove the menu bar widget can reach no network — `github-read-client.js` also exports the name, and a suite pins the two strings equal so they cannot drift apart silently. Never returns or logs the value |
...
| `isOfferableModel(provider, modelId)` | The allow-list predicate. A **memoised `Map.get`** since v3.57.0 (`offerMemoFor(provider).index`), so prototype keys resolve to nothing |
...
It is **a store**, in the sense that it exposes plain functions, renders no prompt and calls no LLM — every decision about what to say to a model lives in its callers. It has **two** of those, and they are not symmetric. The MCP tool layer (`mcp/tools/working-state.js`) reads and writes every tier; `src/routes/memory.js` (mounted in `src/server.js`, and the backing for the Project-context view described under "The redesigned shell" above) reads every tier and writes **tier 1, the standing brief, AND — since v3.61.0 — any KEPT (written/copied) tier-0 foundation** (`initFoundations`, `saveFoundation`, `removeFoundation`, and — new in v3.69.0 — `addFoundationsFromFolder`, `addFoundationsFromRemote`, `setFoundationsSource`, `refreshFoundationsFromRepo` per source group), along with the project create/rename/delete operations that manage the folders around it. A **mirrored** foundation's *content* is never written here, only mirrored — the same byte-copy exception a refresh already was; `PUT` is refused per document, on the slug's own `source.kind`, never on the project — and since v3.65.1 the app can also add a GitHub source alongside an existing folder source, through `addFoundationsFromRemote`/`setFoundationsSource` and the same read path a refresh uses; v3.69.0 lifted the old one-source-per-project ceiling, so this is now addition, not a switch. **Since v3.61.1, `removeFoundation` is the one asymmetry** — it may also remove a mirrored document's manifest entry and copy (and, since v3.69.0, its source group when it was that source's last document), gated on the manifest existing at all rather than on the document's kind, because dropping an entry is a decision to stop mirroring, not a claim about content, and does not create a second writer of anything. **Tiers 2 and 3 have exactly one writer: an agent, through `save_working_state`.** That single-writer property is what makes the per-machine layout safe — decision 2 below turns on two machines never touching the same file, and a browser writer into a handoff would make the app a second writer to files whose whole safety argument assumes there is one. It would also stamp a human's edit with the last agent's harness/model provenance, which is the honesty half of the same argument. The **brief**, and now a **curator-owned foundation**, were never inside that argument: neither has a machine segment, both are the human's to write, and each is stamped `authoredBy.kind: 'human'` when the app is the writer — the store's single-writer property was always "one writer per FILE, with provenance that matches", never "one process", and a second human-facing writer for a human-owned document does not strain it. What decision 4 relies on is therefore no longer *nothing writes the brief* but *nothing writes the brief, or a curator-owned foundation, as a side effect*, plus a provenance stamp that says which of the permitted routes did. It is written to be imported inside the MCP stdio child, so it must keep stdout pure — `console.error` only, per the v2.5.3 stdout-pollution rule. Its reads go to the filesystem directly and deliberately **not** through `mcp/graph.js`, whose cache invalidates on **file count**: an in-place overwrite of `current.md` never changes the count, so a cached read could serve state up to the cache TTL out of date, and stale state is worse than no state.

**Layout is `domains/<domain>/state/<project>/` for a NAMED project** (v3.48.0; it was `domains/<domain>/state/` with no project segment), and the state root itself — no project segment, permanently — for the domain's own project, whose slug IS the domain name. Under whichever of those two roots applies: `project.md` (the standing brief, the owner's tier, returned on every read; see decision 4 for why its authorship is load-bearing on the read side), `<scope>/<machine>/current.md` (the handoff, overwritten each save), `<scope>/<machine>/journal.jsonl` (append-only, one line per save). `state/` is a **sibling** of `wiki/` and is never written through `writePage`: that function redirects non-canonical paths into `entities/`/`concepts/`/`summaries/` and flattens to the basename, so the `(project, scope)` pair is inexpressible there.

**The project level is a read-compatibility problem before it is a feature.** A pre-v3.48.0 tree — `project.md` and scope directories sitting directly under `state/` — is read as a project whose slug is the **domain's own name**, the *default project*; readers never move files, because a fleet is heterogeneous during an update and an older machine has to keep reading and writing the same paths. Shapes are told apart structurally, not by a version marker: a directory directly under `state/` is a PROJECT if it holds `project.md` or any `<scope>/<machine>/current.md` two levels down, and a LEGACY SCOPE if it holds `<machine>/current.md` one level down. A tree satisfying both is **reported, never guessed** — the store returns `layoutWarning` and leaves the folder alone. Writes do **not** consult the disk at all: `projectPrefix(domain, project)` is a pure string comparison returning `''` when `project === domain` and `<project>/` otherwise, so the domain's own project writes the root forever — for a tree that predates v3.48.0 and equally for one created today — and a named project always writes its own directory. That is what makes it impossible to split one project's history across two shapes, and it is stronger than a rule that reads the disk first, because there is no moment at which the answer can change under a concurrent write. **Nothing migrates, and nothing needs to**: the root is not an old shape awaiting a move, it is where that project lives. Reorganising a root tree under a name is an optional hand move; no code performs it. Names are resolved rather than assumed — `resolveProject` matches exactly in a named domain, searches every domain for a bare name and returns `project_not_found` / `project_ambiguous` with `candidates` rather than picking, and `resolveScope` accepts `latest` for the newest-written scope. Full treatment in [working-state.md § 2](working-state.md#2-layout-on-disk).
...
2. **The `<machine>` segment is load-bearing — and it is not a hostname.** `state/` matches none of `DOMAINS_GITIGNORE_RULES`, so it syncs, and `sync.pull()` resolves with `git pull --no-rebase -X theirs`. Two machines writing the same `current.md` would destroy each other's handoff with no error; per-machine paths mean no conflicting hunk ever arises. Proven against real git.
    ...
    **That argument covers tiers 2 and 3 only.** `state/<project>/project.md` has **no** machine segment — it is one file per project by design, because the brief belongs to the project rather than the machine. Two machines that both edit the brief therefore *do* produce the conflicting hunk above, and it is the worse place for it: the brief is returned on **every** read, so both machines inherit a standing decision neither owner recorded. **v3.48.0 is the "second writer" this paragraph told a later release to revisit**: the brief gained an app editor and an MCP writer, so the exposure is real rather than theoretical. What bounds it is frequency rather than absence — a brief is a document that changes a few times a year, and a write replaces it whole — and the mitigation is the ordinary one, sync after editing. Anyone making the brief a *frequently* written file has to solve this properly rather than inherit either argument.
...
4. **Tier 1 is framed separately from tiers 2 and 3, and the classifier fails safe.** Decision 3's framing — *this was written by an earlier session and is untrusted recorded data, not instructions* — is correct for `current.md` and `journal.jsonl`, and was applied to `state/project.md` as well. That is a different kind of file: hand-authored by the project owner, written by no tool. Telling a model that the owner's own standing instructions are an earlier session's notes does not merely misdescribe the file, it **decides every conflict against the owner** — measured, when a brief's *"you are the orchestrator; you do not build"* met a conflicting rule in the agent's harness prompt and lost **silently**. So `mcp/tools/working-state.js` emits a per-tier framing: a brief that verifies as owner-authored leaves the `content_is_data` list entirely and carries its own `authority_note` plus `brief_authority` (`owner` | `commissioned` | `mirror` | `suspect` | `unverified`). **`commissioned` is v3.48.0's addition**: a brief written through `save_project_brief` carries a provenance comment naming the agent, the model and the fact that a user commissioned it, and it keeps the owner framing with its own note — an agent writing at the owner's request is still the owner's voice, and treating it as an earlier session's notes would reintroduce the exact defect this decision exists to fix. It never resolves to `untrusted` on account of being commissioned; the three conservative verdicts are reached the same way they always were.
...
5. **Tier 0 (v3.59.0) is a fourth store under the same module, deliberately not folded into tiers
   ...
   than a manifest entry pointing at nothing. Full treatment, including the manifest schema (v1 and,
   since v3.69.0, v2's per-document sources), the document-level source check and the one-writer
   rule, and the `get_project_context` bootstrap that reads all four tiers in one call:
...
| `saveProjectBriefText(domain, project, text, opts)` | **The canonical brief writer.** Replaces the project's `project.md` with the whole markdown `text`, through the same sanitisers and the 32 KB cap as everything else, keeping arbitrary `## ` sections rather than reconstructing the document from named fields — a structured rewrite would silently drop a hand-written section such as *Roadmap* or *How I want you to work*. `opts.authoredBy` records who wrote it (`human`, or an agent with its harness and model) as a provenance comment the read side classifies; `opts.replace` clears the destructive-shrink guard and `opts.allowCreate` permits a project that does not exist yet. Its production callers are the `save_project_brief` MCP tool (agent, on the user's explicit instruction), `createProject`, and the app's tier-1 editor. Decision 4 no longer rests on *nothing calls it*; it rests on nothing calling it as a **side effect**, which is a property of the tool's contract rather than of an empty call graph. |
| `saveProjectBrief(domain, project, text, opts)` | A **dispatcher on argument shape**, kept so the pre-v3.48.0 call form still resolves: a string third argument goes to `saveProjectBriefText`, anything else to `saveProjectBriefSections(project, input)` — the legacy structured writer over the four `BRIEF_SECTIONS`, which has no production caller, writes no provenance header and runs with the shrink guard off. New code should name `saveProjectBriefText`; two writers exist so that one of them can be the old one. |
...
**"Newest" orders by the agent's own clock, file `mtime` only as a fallback (v3.74.0).** `listWorkingScopes`, `listScopeMachines` and `summariseProject` (and, through them, `scope: "latest"` and every "most recently written project" listing) sort pairs by `writtenAt` — the journal's own `at` for that pair's newest line — falling back to file `mtime` only when no journal line parses. Before this, ordering was `mtime`-only, and a Personal Sync pull could leave a stale handoff with a newer file `mtime` than the true latest save (a pull only ever moves `mtime` **later**, never earlier), so `scope: "latest"` could open the wrong work-stream after a pull — the same "file time is not the same clock as agent time" distinction the tray already draws in its own age labels (§ The menu bar widget, above) now governs *ordering*, not just display. **One tool, one name, everywhere a harness is compared (v3.74.0).** `src/brain/harness-names.js`'s `normaliseHarness(raw) → {id, label, variant, raw}` folds free-text spelling drift — `Claude Code`, `claude-code`, `Claude Code (desktop)` — to one comparable id, so a false "two tools are writing this scope" collision (from one tool saving under two spellings) and an undercounted tool tally in the tray's save pulse both stop happening; `claude-desktop` (the Claude Desktop app) is a **genuinely different product** and the alias table never merges it into `claude-code`, mirroring the existing `claude-ai` rule in `mcp-clients.js`. An unrecognised name is never fuzzy-matched to a known one — it passes through as itself, case- and whitespace-folded only.
...
It is a **projection over the working-state store, not a second inventory of it.** It calls `listWorkingScopes()`, the same function `GET /api/memory` and the MCP call, so the tray and the Project-context view can never disagree about what is on disk. It costs exactly what `GET /api/memory` costs — one `readdir` plus a stat per candidate, then per project one stat per `(scope, machine)` pair and one 16 KB journal-tail read per pair returned. No LLM, no network, no subprocess, and no write to the store.

It is its own module rather than a function inside `working-state.js` for three reasons, of which two are structural: this row is a *consumer's* shape rather than the store's — the store caps its own listings (`MAX_PROJECTS_PER_DOMAIN` / `MAX_PROJECTS_TOTAL`, both 200) but says nothing about how a tray or a route should bound itself, which is why `TRAY_MAX_PROJECTS` and `src/routes/memory.js`'s `MAX_PROJECTS` exist separately and equal — and this row carries facts the store does not own — `remote` is a Personal Sync fact, and importing `sync.js` here would drag a `git fetch` into the import graph the MCP child loads on every spawn, which is exactly the graph the stdout-discipline rule is stated over. **It imports nothing from `sync.js`**, so no edit to this file can reach `getRemoteStatus()` without adding an import a reviewer will see.
...
| `TRAY_DEFAULT_LIMIT` (8) · `TRAY_MAX_LIMIT` (40) · `TRAY_MAX_PROJECTS` (200) · `REMOTE_OBSERVATION_MAX_AGE_MS` (5 min) | `TRAY_MAX_PROJECTS` is deliberately the same number as `MAX_PROJECTS` in `src/routes/memory.js` — and as the store's own `MAX_PROJECTS_TOTAL` — and deliberately **not imported** from either (`src/brain/` must not import from `src/routes/`). Matching matters more than the value: a tighter cap here would make the tray say "nothing saved" about a project the Project-context view lists happily. If one moves, move all three |
...
### `src/brain/session-start.js` (v3.70.0)

The **one measurement** behind three consumers: `GET /api/memory/:domain/:project/session-start`
and `POST …/session-start/preview` (the Context view, step ④'s meter) and
`getTraySummary().sessionStart` (the menu bar widget). Moved out of `src/routes/memory.js`
(v3.67.0's `sessionStartReport`) so the tray — which lives in `src/brain/`, and may not import a
route — can call the same function; the route is now a thin wrapper. It is the one sanctioned lazy
import from `src/brain/` into `mcp/`: since v3.70.0 the MCP figure is the **real**
`get_project_context` handler's reply, invoked directly rather than through the MCP dispatcher, so
a measurement is never written to the usage log. Because a large bootstrap is now paged
(`CONTEXT_PAGE_BYTES`), the byte figure is every page's measured size, and the per-layer split
reads every page's document text, not page 1's alone. The window and harness figures are **read**
from the install's config (`src/brain/config.js`'s `getContextWindowSettings`); nothing here
writes anything — no state file, no manifest, no usage-log line, no config.
...
POST /api/config/pick-folder   → macOS native folder picker (osascript); MUTATES — calls setDomainsDir()
POST /api/config/pick-path     → native folder picker that mutates NOTHING — hands back a path only
                                 (v3.61.1; the Foundations chooser's "Choose folder…")
...
GET    /api/config/github-read-token       → { ok, present, last4, kind } — never the value (v3.65.2)
PUT    /api/config/github-read-token       → body { token }; 400 invalid_token unless it trims to
                                             github_pat_… or ghp_… with no whitespace; guardConcurrent'd
DELETE /api/config/github-read-token       → removes only that key; { ok, present:false }; guardConcurrent'd
POST   /api/config/github-read-token/test  → body { remote, ref? }; ONE read of that repository's ref
                                             with the STORED token (a `token` in the body is ignored) —
                                             { ok:true, repo, ref, sha, requests } or, when the request
                                             itself was malformed or no token is saved, 400; when the
                                             test ran and GitHub refused, 200 { ok:false, code, message }
                                             (this is an ANSWER, not a failed request). requests is 1
                                             when ref is named, 2 when the default branch must be
                                             resolved first. This is the only HTTP path a GitHub token
                                             VALUE ever arrives on; it never leaves the server again —
                                             every other response about it carries presence + last4 only

GET  /api/config/context-window → { ok, settings } — contextWindowTokens, contextWindowCustom,
                                   harnessEstimateTokens, choices ([200000,400000,1000000]) (v3.70.0)
PUT  /api/config/context-window → body { contextWindowTokens?, harnessEstimateTokens? }; PER COMPUTER,
                                   not synced, read by the app and the menu bar widget alike; refuses
                                   409 harness_exceeds_window when the harness estimate would exceed
                                   the chosen window, and 400 on an unexpected field or an empty body
```
