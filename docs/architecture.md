...
│   │   │                       init + human PUT/DELETE + a widened repo-scan — v3.61.0). Never
...
│   │   │                       both stamped authoredBy.kind: 'human'; a repo-owned document is
│   │   │                       never written here, only mirrored, which is why a mirror refresh
│   │   │                       stays the one legitimate write outside those two — see below).
