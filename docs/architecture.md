...
│   │   │                       (reads all four tiers, writes tier 1 AND curator-owned tier 0 —
│   │   │                       both stamped authoredBy.kind: 'human'. A repo-owned document's
│   │   │                       CONTENT is never written here, only mirrored — PUT still refuses
│   │   │                       one outright — but DELETE may remove a repo-owned document's
│   │   │                       manifest entry and copy since v3.61.1, which is a decision to stop
│   │   │                       mirroring it, not a claim about its content; a mirror refresh stays
│   │   │                       the one legitimate CONTENT write outside the curator-owned two —
│   │   │                       see below). Five more surfaces READ it and write nothing:
