...
| `saveProjectBrief(domain, project, text, opts)` | Replaces `state/<project>/project.md` with the whole markdown `text`, through the same sanitisers and the 32 KB cap as everything else, keeping arbitrary `## ` sections rather than reconstructing the document from named fields — a structured rewrite would silently drop a hand-written section such as *Roadmap* or *How I want you to work*. `opts.authoredBy` records who wrote it (`human`, or an agent with its harness and model) as a provenance comment the read side classifies. It has three callers as of v3.48.0 — the app's brief editor (`human`), the `save_project_brief` MCP tool (agent, on the user's instruction), and a text editor, which is not a caller at all. Decision 4 no longer rests on *nothing calls it*; it rests on nothing calling it as a **side effect**, which is a property of the tool's contract rather than of an empty call graph. |
