...
`app.js`/`index.html`/`styles.css` were held byte-identical for the whole `/next` build-out, so redesign work could never blank the shipping app. That held until v3.6.0, which made a deliberate 26-line change to four `app.js` handler bodies (a refused destructive write was rendering underneath an opaque modal). `index.html` and `styles.css` remain byte-untouched, and the rule that nothing is added at **module scope** — the failure mode that blanks the app for every user — is unchanged.
...
│           │                    markdown.js — the ONE /next Markdown renderer, used by chat.js + domains.js
