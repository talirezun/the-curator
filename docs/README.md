...
| [dev/](dev/README.md) | The development guide's full text — decisions (product/UI, wiki pipeline, agents), the wiki pipeline reference, the directory map, operations and the release index — moved out of the auto-loaded `CLAUDE.md` **verbatim** so the auto-loaded file stays small |
| [../CHANGELOG-ARCHIVE.md](../CHANGELOG-ARCHIVE.md) | **This project's memory.** Every release row older than the newest three, moved out of `CLAUDE.md` **byte-for-byte** — the measurements, traps and "KNOWN AND UNFIXED" items that explain why the code is shaped the way it is. `CLAUDE.md` keeps the newest three in full plus a one-line index of every entry here, in [dev/release-index.md](dev/release-index.md). Read the full row before changing anything with a history. It lives at the repo **root**, not in `docs/`, and that is load-bearing — the preserved rows carry root-relative links; see the note at the top of the file |
...
| [roadmap-menubar-widget.md](roadmap-menubar-widget.md) | The design pass behind the Mac menu bar icon. **Phase 1 shipped; Phase 3 partly (an aggregate save pulse, and since v3.74.0 a "Saves by tool" submenu); Phase 2 did not.** §0a is the exact boundary — what is built, the twelve places the build deviated from the plan and why, and what has never been rendered. The rest is the reasoning, deliberately left as written |
