...
This is the connected state you are aiming for — the bridge reporting back how many tools it exposes and how many domains it can see:

![The Curator's Settings screen, MCP bridge section. A green Connected line reads "Claude Desktop → my-curator →" followed by the knowledge folder path (blurred here). Below it: "Works with any MCP client that runs local servers — Claude Desktop, Claude Code, Cursor, and others. ChatGPT's web app cannot run a local server, so it cannot connect." Four actions — Re-run setup, Run self-test, View config, Copy snippet — sit above a "Bridge responds" result reading "22 tools (list_domains, get_index, get_graph_overview, get_tags, search_wiki, search_cross_domain, …) · 6 domain(s) visible". At the bottom, a "Default domain for MCP writes" dropdown set to "projects".](images/curator-mcp-bridge.png)
