...
For most second-brain users, the loop is: ingest → admire the Obsidian graph → use the app's own Chat view for everyday lookups. That's already useful. But the graph is something you *look at*. The synapses — the actual connections between thousands of knowledge nodes accumulated over years — are mostly invisible to you while you're inside the graph.
...
My Curator exposes **twenty-four tools** to whichever client you connect — fourteen read tools that explore your knowledge graph, your own prior working state, and (v3.59.0+) a project's canonical foundations, and ten health/authoring tools (v2.5.2+) that let Claude maintain and *update* the wiki and your working state on your behalf. Seven of those ten actually change anything on disk (`compile_to_wiki`, `fix_wiki_issue`, `dismiss_wiki_issue`, `undismiss_wiki_issue`, `save_working_state`, `save_project_brief`, `save_foundation`); the other three only scan and report. Counted the other way round — by what a tool *does* rather than which block it sits in — that is **seventeen that read and seven that write**.
...
| `get_project_context` | The session-start bootstrap (v3.59.0) — the brief, the latest handoff, AND the project's foundations, in one call. Prefer this over `get_working_state` at the start of a session. Since v3.62.0 it returns the **index of every document, always**, and the **text** of the ones you marked *read first*; everything else is an index row Claude opens **by name** with `slugs`. Some of what it returns may be a **skeleton** — a document seeded with prompts rather than facts, e.g. from the [start-a-project flow](user-guide.md#start-a-project) — and the tool says so per document, so Claude reads those as questions you want answered rather than as settled fact. **As of v3.69.0**, a project may mix documents written here, copied in, and mirrored from several folders or GitHub repositories; `foundations.sources` then names each mirror source and each document's `source.group` says which one it came from — carried only when a project actually mixes kept and mirrored documents, or mirrors from two or more sources, so an unmixed project's response stays byte-identical to v3.68.0's |
...
| `save_working_state` | Write this session's handoff (Track 7) so the next session — possibly a different tool, model, or machine — can resume cold. An optional `repo_root` (absolute path of a checkout with a `.curator-project` marker) refreshes the documents this project mirrors from that checkout — a folder source inside it, or, **as of v3.69.0**, a GitHub source whose origin it is — advisory, and skipped with a reason otherwise; a GitHub source stays a GitHub source, `repo_root` never turns it back into a folder mirror |
...
1. Open The Curator → **Settings** (rail footer) → **MCP bridge**, and click **Set up Claude Desktop**. (If a bridge is already connected the same button reads **Re-run setup**. Through v3.40.0, in the pre-redesign shell at `/old`, the section was in the Settings tab and headed *My Curator — Private MCP Bridge*. That shell was deleted in v3.41.0.)
2. Step 1 of the wizard, *Copy the Curator entry*: choose **Just the Curator entry** (safe if you already run other MCP servers) or **The whole file, with Curator added**, then click **Copy the Curator entry & continue →**.
3. Step 2, *Paste it into Claude Desktop's config*: click **Show it in Finder** and Finder opens the folder containing `claude_desktop_config.json`. (**Copy the path** is there too, and where the wizard can do it safely it offers **Write it for me**.)
4. Open that file in any text editor (TextEdit, VS Code). If it doesn't exist, create one containing just `{}`.
5. Paste the snippet inside the `mcpServers` key (the wizard shows you exactly what "before" and "after" look like), save, and click **I've saved the file →**.
6. Step 3, *Restart Claude Desktop, then check it*: **fully quit and reopen Claude Desktop** (⌘Q — closing the window is not enough), then click **Test the bridge**. **Re-read the config file** is the separate check of what Claude Desktop now has saved.
7. Close the wizard. The **MCP bridge** section then shows a green **Connected** line, and **Run self-test** re-runs the same check any time. In Claude Desktop:
...
| Move the knowledge folder | **The entry goes stale.** Come back to **Settings → MCP bridge**, click **Re-run setup**, paste the fresh snippet, restart Claude Desktop. The wizard detects the mismatch and shows a banner naming every cause | **Nothing to do.** There is no folder path in the entry, so the bridge resolves the same live Settings value the app does |
...
| "Stale config" banner in Settings | **Source install:** Knowledge Base Location changed since you last generated the snippet. **Packaged app:** a folder change cannot cause this — the entry holds no folder path — so look for a moved app or a hand-edited config instead | Click **Re-run setup**, paste the fresh snippet, restart Claude Desktop. In the app, launching it once also rewrites its own launcher script |
...
- Not a replacement for the built-in Chat view, Obsidian graph view, or the ingest pipeline. It's an *additional* access path: Obsidian gives you spatial exploration, Chat gives you a quick Q&A, and My Curator gives you frontier-model research.
