...
![The Curator's Settings screen, MCP bridge section, as two numbered blocks. Block ① "Connect a client" opens with one sentence — "Works with any MCP client that runs local servers: Claude Desktop, Claude Code, Cursor." — followed by a "Read the MCP guide." link and a small ⓘ help mark. A status card holds an amber "Needs re-connect" pill beside the monospace chain "Claude Desktop → my-curator → ~/Curator/domains" (the real path is long, and is shortened here). Four actions sit under it: a filled "Re-connect", outlined "Run self-test" and "View config", and a plain "Copy snippet". Below a hairline, block ② "Default domain for MCP writes" carries the single line "Used when a client says ‘my wiki’ without naming a domain.", its own ⓘ, and a dropdown set to "demo".](images/curator-mcp-bridge.png)
