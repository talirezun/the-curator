...
- **Every pill shows its whole value** (v3.77.0). When the window is too narrow for all four on
  one row — a long domain or project name, or a window under about 1280 pixels — a pill moves to a
  second row whole rather than being cut to *"Early Computi…"*. On a wide composer the Model pill
  drops its leading word "Model" (the model's name says what it is) so the usual case still fits on
  one row; the word comes back when the composer is narrower. Only a name wider than the whole
  composer is ever shortened.
