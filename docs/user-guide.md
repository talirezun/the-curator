...
You'll see the The Curator interface with seven tabs at the top, in this order: **Domains**, **Ingest**, **Chat**, **Wiki**, **Health**, **Sync**, and **Settings**.
...
**6. You can close the tab — or even restart the app.** The batch lives on the server, not in your browser tab, so closing the tab doesn't stop it. Come back later, or just reopen the Ingest tab, and The Curator shows you the batch exactly where it stands, reattaching to the live progress if it's still running.
...
Click **Scan** in the Health tab, then click **✨ Find duplicate pages** in the **⚡ Quick maintenance** bar at the top of the results (it appears whenever an API key is configured — including on a wiki that is otherwise structurally clean). Before v3.0.1-beta.17 this was a standalone "Scan for semantic duplicates" card below the report; it now lives in the maintenance bar. This finds pages that the algorithm can't catch — like `[[rag]]` and `[[retrieval-augmented-generation]]`, or `[[email]]` and `[[e-mail]]`, or `[[neural-network]]` and `[[neural-networks]]`.
