...
- **A context window below 32,768 tokens** — and, separately, **below 131,072 for the BUILD lane**. Both figures are **derived from the app's own budgets** (v3.45.0), not from a model we happen to ship.
  - **Build: 131,072.** The measured working set is an 80,000-character `TEXT_CAP` plus the index and slug inventory — a real outline prompt of ~341,005 characters ≈ **85,000 tokens** — plus `MULTI_PHASE_OUTLINE_TOKENS` = **24,576** of output: **109,576 tokens** the model must hold at once. 131,072 is that plus ~19.6% headroom, and the first catalogue tier boundary above it.
  - **Chat: 32,768.** `CONTENT_BUDGET_CHARS` 60,000 + `CATALOGUE_BUDGET_CHARS` 12,000 ≈ 18,000 tokens, plus the 8,192-token output request ≈ **26,000**. 32,768 is the tier boundary above that.
  - **Only the chat floor REJECTS.** It is the admission gate, because every fetched entry is admitted into the chat lane by construction. The build floor is a **facet**: it sets a `lanes.build` flag and contributes no rejection reason. Making it a second gate would hide chat-capable models from chat — the single-floor defect one level down.

  **This replaced a parity rule** — *not worse on context than `claude-haiku-4.5`, which publishes 200,000* — that was simultaneously too high for build and far too high for chat, and that **ejected the app's own OpenRouter fallback rung** (`ibm-granite/granite-4.0-h-micro`, 131,000, measured 9/9 clean). Measured on the pinned 2026-09-02 snapshot of 421 records: the old gate admitted **162**; the new one admits **218** for chat, of which **201** clear the build floor.

- **The hand-typed tables are held to the same rules (v3.45.0).** `OFFERABLE_MODELS` and `FALLBACK_CHAINS` used to be exempt *structurally* — nothing ran them through the filter — which is how a fallback rung came to fail the app's own floor unnoticed. `auditStaticOffers()` now runs every static offer and every rung through `checkStaticEntry`, applying the four rules that are decidable from a hand-typed entry (moving alias, batch-only, output ceiling, context window) and **naming the four that are not**, with the reason. Anything that falls short must carry an explicit entry in `STATIC_ELIGIBILITY_EXEMPTIONS`. **There is exactly one**: granite, at 131,000 against a 131,072 build floor — 72 tokens short of a rounded tier boundary while clearing the *measured* requirement by 21,424. Direct evidence from the real workload outranks a proxy for the absence of it; lowering the floor to admit one favourite would destroy the derivation, so the disagreement is written down instead.
...
| `qwen/qwen3-30b-a3b-instruct-2507` | **Context floor.** Measured clean and the fastest of everything tested (13–22 s), price exact on all 8 runs that started. But the endpoint that bills it at $0.04815 carries a **128,000**-token window; the endpoints offering 262,144 cost 2–3× more. The cheap price and the large window are not available at the same time. *(Recorded against the 200,000 floor of the day. Under v3.45.0's per-lane floors it is CHAT-eligible and still short of the 131,072 build floor by 3,072 tokens, so the refusal for the build lane stands — on a much smaller margin, and for a reason now stated on the row rather than by omission.)* |
...
- **Saving an OpenRouter key does NOT make it the active provider** — as of v3.45.0 no key save moves the build lane once one is set. See the section immediately below.
...
#### Saving a key does not change which model builds your wiki (v3.45.0)

**`activeProvider` now has exactly one writer after the first key: choosing a build model.** On disk nothing changed — `activeProvider` plus `selectedModels[activeProvider]` is still the storage for "whose model builds the wiki", and `getProviderInfo()` resolves it exactly as before. What changed is who may write it:

- **The first key you connect takes the build lane.** There is no prior answer to overwrite, and adopting it is the helpful thing.
- **Every later key save leaves the lane alone.** The key is saved; the bill does not move.
- **`POST /api-keys/build-model` is the only thing that moves it afterwards.**
- **Disconnecting the active provider** hands the lane to the **cheapest measured** provider still connected, and the response says which and under which rule (`cheapest_measured` when price ordering actually governed, `first_connected` when there was nothing to order).

**Why.** Under the old *last-saved-wins* (v2.4.2–v3.44.0), pasting an OpenRouter key to try a model in chat silently moved your next ingest from `gemini-2.5-flash-lite` to `upstage/solar-pro4` — a different model, from a different vendor, at a different price, chosen by nobody. `docs/user-guide.md` already called that the likeliest surprise in the whole router. The cost of the fix is one extra click for someone who *deliberately* pasted a key in order to switch providers; that is a knowing trade, and it is the direction where a mistake is visible rather than silent.

**Reversibility.** The policy is a single constant, `ACTIVE_PROVIDER_DERIVED` in [`src/brain/config.js`](../src/brain/config.js). Setting it to `false` restores v2.4.2 behaviour exactly: no field changes shape, nothing migrates, `getProviderInfo` is untouched. It is deliberately not a user setting — two behaviours a user can toggle would need two sets of documentation for a question they should never have to think about.

**What the API reports.** `POST /api/config/api-keys` returns `activationPolicy` (`'derived'` | `'last-saved-wins'`) on every save, plus `activationDeferred: [{provider, activeProvider}]` naming any key that was saved and deliberately did not take the lane. That is kept **out** of `skippedActivation`, which means *could not be activated* and renders as a warning — a working policy announced as a warning on every second key save teaches users to ignore the array that carries the real failure.

**Chat is a separate lane and is unaffected**, as before. Chat sends an explicit per-call model and never reads the pinned default.
