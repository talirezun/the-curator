...
*The demo project's Project context with the **Handoffs** row open: one handoff per tool — `claude-code` and `antigravity` — plus a shared `main` scope where Antigravity's save replaced Claude Code's (the reader keeps the previous one). Each row ends in the row trash, which moves that handoff to **Settings → Trash** after a typed confirmation. Shown on the synthetic demo workspace; `node scripts/screenshots.mjs` regenerates it.*
