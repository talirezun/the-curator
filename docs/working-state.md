...
domains/<domain>/
  ...
  state/
    ...
    <project>/foundations/                     Tier 0 (v3.59.0) — canonical documents + manifest
    ...
    <project>/project.json                     Curator metadata (v3.65.0), not a tier — today
                                               just knowledgeDomains; written by the app only

    project.md                                 …and the DOMAIN'S OWN project, at the root,
    foundations/                               with no project segment at all
...
path segment, lowercase, `a–z 0–9 . - _`, at most 64 characters, no `..`. The store reserves
five names, one for every file or directory it addresses inside a project: `project.md`,
`project.json` (v3.65.0), `journal.jsonl`, `current.md` and `foundations`. So is **the domain's own name**, because
...
| `get_project_context`'s own reply guard | 300 KB (`RESPONSE_BUDGET_BYTES`) | Under the general 400 KB MCP cap below; a reply this large is now paged (next row) rather than degraded in place |
| Delivery, per page (v3.70.0) | ≈80 KB (`CONTEXT_PAGE_BYTES`, ≈20k tokens) | A bootstrap whose measured reply exceeds this is split into whole-document pages, in reading order; page 1 always carries the brief, handoff, journal and index. See [Paged delivery](#paged-delivery-v3700), below |
...
**The reading budget itself (v3.67.0), `manifest`-level, not per-document — a 7-preset token ladder
since v3.70.0.** `readingBudgetBytes` is `0` (Index only) or `8,192`–`819,200` bytes
(`CONTEXT_MAX_BYTES_CAP` moved 200 KB → 800 KB); absent means **not set**, and the project behaves
...
`readingBudgetPreset(bytes)` names which of the seven presets a stored value **is** — Index only
(0) · Lean (8,192 B) · Standard (65,536 B) · Deep (131,072 B) · Large (262,144 B) · Extra large
(524,288 B) · Max (819,200 B) — or, for any other valid value (an old 120 KB Deep or 200 KB Max, or
a hand edit), `{id: 'custom', nearest}` naming the closest preset. It never writes; it is read
by the app's picker to label a stored value.

#### Paged delivery (v3.70.0)

Claude Code shows an MCP reply of at most ≈25,000 tokens by default and **saves a larger one to a
file**, handing the model a file reference instead of the text — so a bootstrap that fit the old
120–800 KB ceiling was not necessarily inside the model's actual context window. `get_project_context`
now bounds **the whole serialised reply**, not document text alone, at `CONTEXT_PAGE_BYTES` (≈80 KB,
≈20k tokens — 20% headroom under the 25k cap for a denser tokenizer than the byte-÷-4 estimate).
Page 1 always carries the brief (≤32 KB), the handoff (≤48 KB), the journal and the full index —
before a single byte of document text — so 80 KB of documents on top of those would already be over
the cap this page bound exists to respect. A reply that fits in one page is returned **exactly as
before v3.70.0**, byte-identical; only a reply that would exceed ≈80 KB is split. Each page but the
last carries `foundations.continuation: {page, of, remaining, slugs}` naming the next page's
documents; the `report` field states the next call in words. **A new `page` argument** (a whole
number ≥ 1; anything else is refused) returns that page, using the same `slugs` machinery — no new
read path. Whole documents only: a document larger than one page arrives alone on a page of its
own, still bounded by the 300 KB `RESPONSE_BUDGET_BYTES` guard; one that exceeds even that is
listed in the index, never sent. Read **every** page before starting work, and do not pass a later
page's `seen` back as `seen_hashes` on the next call — the skill and `get_project_context`'s own
description both carry this rule, and the maintainer must re-upload the skills by hand after any
...
still shows the earlier screen and is due a re-shoot. It also predates **v3.65.1**, which renamed
*Foundations/Working state/Work-streams/Recent saves* to *Documents/Memory/Handoffs/Journal*,
removed the **Last saved** row (its fact now lives in the overview's MEMORY tile and in the
Handoffs row's own summary), gave every domain named on the screen its own identity dot, moved the
Knowledge picker into a head row with a per-row **Remove**, and added the **"Mirror from GitHub
instead"** control to step ①.*

**What the view puts in front of you (rebuilt in v3.55.0, finished in v3.56.0, renumbered in
v3.62.0, unified with the Domains page in v3.65.0).** It was three collapsible panels under a row of
dropdowns; then a dashboard of unnumbered blocks. It is now **three numbered steps**, read top to
bottom, under the **overview card** — since v3.65.0 the very same `renderOverview()` component the
Domains page draws, at the same tile geometry and the same figure size. Each numbered title carries
its ⓘ beside it and **no lede sentence underneath**; the explanation lives in the ⓘ:
...
| ② | **Memory** *(the store still calls this* working state *)* | Four closed rows, in this order: **Agent connections** (called *Agent sessions* through v3.73.x, *Capture* through v3.69.0; the on-disk word — route `…/capture`, `captureFacts` — is unchanged), **Handoffs** (a press opens that handoff in the reader; the store calls each one a `scope`), **The brief** (yours, with a pencil), **Journal**. Everything that qualifies them sits unfolded, outside every row. **v3.65.1 removes the "Last saved" row** that used to lead them; its fact moved into the overview's MEMORY tile and into the Handoffs row's own summary |
...
**The overview card, not a strip and not a block.** It sits between the project breadcrumb and step
① and carries **four tiles** — **DOCUMENTS · MEMORY · KNOWLEDGE**, the same nouns the three
steps use, plus a fourth (**CAPTURE** through v3.69.0, **AGENT SESSIONS** through v3.73.x, **AGENT
CONNECTIONS** since v3.74.0 — the jump id, the route and every on-disk name keep the old word) —
each on the app-wide freshness scale, and an unknown age is a **dashed
ring and the words**, never age zero. Pressing a tile scrolls to the step it names and leaves focus
on that step's own heading. Through v3.64.2 it was a three-cell strip with the jump tiles drawn at a
second, smaller geometry; v3.65.0 retired that second design, so a Context tile and a Domains tile
are now one object with one host-settable track floor (253px here, 175px there). It replaced a
*Status* block that was itself the "everything mixed together" problem: a tier-0 reading, a tier-2
reading, four tier-2 warnings and a tier-1 line under one heading called *Status*. Its warnings did
not disappear with it — each one moved to the step it qualifies, where warnings, costs, refusals and
outcomes are never folded. **As of v3.65.1 the breadcrumb above the card also carries the project's
domain's own [identity dot](design-system-source.md#18-identity--one-palette-one-mapping-one-glyph-v3651)** — the same colour that domain has on the
Domains sidebar, the Context sidebar and every Knowledge row below — where it used to be a plain
violet square unrelated to any domain.

**Every fold ships closed, and the state is remembered per fold.** That is v3.58.0's measurement
applied to the whole page, and each summary line carries the figure that decides whether to open it.
The three no-op guards the page has always had were re-measured against the renumbered structure and
hold: a switch back to a project already read replaces the view root **once**, pressing a Handoffs
row repaints **nothing**, and ticking a document read-first repaints nothing either — the row, the
...
unfolded and step ② renders a flat card saying nothing has been saved, because the missing thing has
to be missing where you looked for it.

**The two clocks, and what "saved" does not claim.** Every age on this page comes from one of
two clocks, and the overview card's ⓘ says which. The **agent's clock** is the time the agent itself
...
in words, rather than in a tooltip. And *"saved"* is exactly that: it knows when the last save
...
|---|---|
| **The overview card**, tile ① | *DOCUMENTS* — the count and the freshness word, on the shared scale; `no documents yet` when there are none. **It is omitted entirely while the project read is in flight**, rather than guessing a figure it does not have yet — a recorded trade: one fewer reading for a moment, against a number that would have been wrong |
| **The overview card**, tile ② | *MEMORY* — `saved 14 min ago`, from the newest save across every scope; `nothing written yet` when there is none. This is the *"am I saved?"* answer, asked by somebody with almost no context left, and it is one line rather than a block — as of v3.65.1 it is also the ONLY place this fact appears above the fold, now that the "Last saved" row is gone |
| **The overview card**, tile ③ | *KNOWLEDGE* — the page count and the last ingest's day-age |
| **The overview card**, tile ④ | *CAPTURE* — `1 session in the last 30 days`, the honesty meter's headline (v3.63.0), promoted onto the card in v3.65.0. It joins the row once its own request has landed, and never before |
| **Step ① Documents** | A table of the project's canonical documents, eight columns as shipped on the repo-owned arm: ROLE · DOCUMENT · SIZE · **READ** · SOURCE · COPY · UPDATED, plus an eighth whose heading is visually hidden because it holds an icon-only control — the per-row *Remove*. A curator-owned project collapses SOURCE and COPY into one STATE column and comes in at seven. The READ cell is the read-first toggle, a pill reading `read first` or `on request` and carrying `aria-pressed`. **As of v3.65.1 the SIZE cell also carries a [depth bar](design-system-source.md#19-the-depth-bar-v3651)** — a tinted bar behind the figure, anchored right, its length that document's share of the 200 KB project budget; a document that alone exceeds the budget fills the cell in the warning tone, and the total-over-budget warning under the table still states the same fact in words. A row press opens the document in the reader. The summary line counts them — *"3 documents · 391 KB · mirrored · 2 read first · 1 on request · fresh"* — and the *Refresh from repo* control is **withheld with its reason** when the project is curator-owned or the checkout is not on this machine. Before ownership is chosen the table is replaced by the question that chooses it |
| **Step ② Memory**, row 1 | **Agent connections** (*Agent sessions* through v3.73.x, *Capture* through v3.69.0) — the honesty meter (v3.63.0), a row since v3.65.0, now leading the four rows since "Last saved" left. Its summary is the headline plus up to three clauses (*started with the context* · *saved before stopping* · *read and did not save*); **as of v3.65.1 its body is the monitor only** — up to six lines, in this order: sessions · started with the context · saved before stopping · read and did not save (warning tone when non-zero) · the tool calls made across the window · the newest session's age with its harness as a sub-line — and **no table**. The route's own note (gated a second way this release — see below) and the window's limits stay **outside** the row, unfolded, and the ⓘ that used to float alone above the table moved into the step's own ⓘ. The word *Sessions* is no longer a heading anywhere on this screen |
| **Step ② Memory**, row 2 | **Handoffs** — one row per **(scope, machine)** pair, newest first: freshness dot and slug, that save's own headline, its age, the machine (tagged **this machine** only on positive evidence) and the harness · model. The newest **five** are painted, with a *Show N more* row under the table that appends the rest; pressing a row opens its handoff in the reader. Under the table, *"N handoffs · M saved copies"*, both taken from the store's uncapped counts, plus *"showing 5 of 16"* — counted in ROWS, so its second figure is the saved-copy total — while the window is short of the list |
| **Step ② Memory**, row 3 | **The brief** — *"updated 14 min ago · 126 words"*, with an icon-only **pencil** beside it — no "Edit" word, no toolbar line above it. Opening the fold reads the document; pressing the pencil opens the editor in its place — ⌘S / ⌘↵ save, Esc closes (raising an inline Discard / Keep editing bar when the draft changed), and a live *modified · words · bytes of 32768* line disables Save before the 32 KB ceiling is hit rather than after |
| **Step ② Memory**, row 4 | **Journal** — *"17 saves · showing 10 · latest 14 min ago"*. Opens to the one-line-per-save list, newest first, with the same inline **"Show N more"** the Handoffs row uses — **v3.65.1 deleted the separate footer card and floating button** it used to end in, which reset the reader's scroll position on every press |
| **Step ② Memory**, notices | Up to three unfolded blocks above the four rows, each rendered only when it has something to say (a healthy save renders none of them, and step ② opens directly on Agent connections) — v3.16.1's rule, unmoved: **"Warnings about the last save"** (renamed from "About the last save" in v3.65.1) carries outcomes about one specific save — trimmed content, a clipped label, a deliberately replaced handoff, two tools sharing one file, newer state in another scope, another machine that saved after this one; a **Reload** notice for a save that landed since the page loaded; and a note for state on disk this read could not list by name. **Two facts left this monitor in v3.65.1, not dropped, moved:** which clock an age came from is now the overview card's own ⓘ, and which machine wrote the open handoff is now the Handoffs table's own MACHINE column per row plus a chip in the reader |
| **Step ③ Knowledge** | **One row per domain the project draws on** (v3.65.0; the project's own domain always an explicit row from v3.65.1), each summarised as *"391 pages · 3 days ago"* with that domain's own identity dot before the name, and opening to five figures — pages · entities · concepts · summaries · last ingest, the last carrying the verb and the source title (*"Ingested · The Energy and Water Footprint of Generative AI"*) — from **one** `GET /api/domains/:domain/stats` each, plus two doors, *Open in Domains* and *Ask this domain*, and its own **Remove** (moved inside the row in v3.65.1, the same place the Documents table's Remove sits). **As of v3.65.1 the entities/concepts/summaries figures each carry a depth bar** against that domain's own page count — the three sum to at most the page count, which is what makes it an exact denominator. A **+ Add a domain** picker sits in the head row above the rows (moved there from under the last row in v3.65.1), one domain at a time, up to **12**; removing the last added domain returns the project to just its own. **A small `default` badge marks a row while nothing has been explicitly chosen yet** — the same quiet-badge class the Documents table uses for "shared mirror" — and it disappears from every row the moment you add one, because the set is then a choice rather than a default; the forty-word paragraph that used to sit under the picker explaining this moved into the step's own ⓘ. Before v3.65.0 this was a single fold, always about the containing domain, with no control at all. No page list, no health report; the view pays no cold health scan to draw a summary |
...
scope and machine as chips, the same *Saved* reading the overview card gives — computed
...
**step ② WORKING STATE**. Through v3.64.2 it sat inside the step but outside the three-cell strip,
because that strip was three cells for three layers and a fourth would have broken the mapping;
v3.65.0 replaced the strip with the overview card, where a fourth tile carries no such claim, so
**CAPTURE is now both** — a tile on the card that scrolls to step ②, and, inside step ②, the second
of the step's five rows.

On screen the row's summary is `CAPTURE · 6 sessions in the last 30 days` — with the
uncomfortable number beside it in words: *"4 started with the context · 4 saved before stopping
...
The row's own body (`data-mem-fold="capture"`, closed) lists the sessions, newest first — started ·
