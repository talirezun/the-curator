...
**Since version 3.66.1 the menu also draws small depth bars**, each one the widget's own twin of an app reading so a fact is never widget-only: each **project** header shows sessions that saved a handoff in the last 30 days against the busiest project (its app twin is MCP bridge's Across projects); a **Domains · pages** section shows each domain's page count against the largest domain, in that domain's own colour (its twin is Knowledge base's "Domains in this folder"); the open project's **documents** line, under its own header, shows document size against whichever budget applies — 120 KB once anything is marked read first, otherwise the project's own stored total, shown plainly, never as an alarm since v3.70.0 (its twin is the Context view's documents monitor); and, since v3.70.0, a **Session start** line directly under it draws the same context-window meter step ④ does, at menu-bar scale — window, harness estimate hatched, and The Curator's own layers. A bar turns red, and says "over," only on an actual over-run; with no usage log on this computer a project line reads "no logged sessions · 30 d" instead of a bar drawn at zero.
