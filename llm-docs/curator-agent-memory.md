...
An existing project that already has its documents is not stuck asking an agent to paste them in one at a time. Four ways an existing document gets into a curator-owned project: ticked from a folder scan and copied in through the **Add from this computer** door (since v3.68.0, appended, never replacing what is already there); mirrored from a repository via **Add from GitHub**; chosen from a file on your own computer inside the **Write a document** editor, read in for you to review before saving; or written by an agent, only when you have asked it to.
...
On a curator-owned project, each row in the Documents block carries an Edit control. The block's own head row carries **Write a document** (renamed from Add document in v3.68.0). Edit opens an editor in place of the table, the same pattern the standing brief already uses. Saving is disabled past 512 KB — a canonical document cannot be honestly trimmed — and a project nearing its 200 KB total budget is told so without being stopped. Filling in a skeleton and saving it clears the skeleton mark for good.

**Since v3.68.0, step ①'s head row always shows two doors — Add from this computer and Add from GitHub — whatever the project's current state, at 0 documents and at 20.** What each one does depends on the project's one source: on an empty project either door sets it (copy in, or mirror from GitHub); on a curator-kept project, Add from this computer copies files in, appended, never replacing what is already there, while Add from GitHub is disabled ("a project has one source"); on a folder mirror, Add from this computer mirrors more from inside that same folder, while Add from GitHub switches the source to a repository (this was "Mirror from GitHub instead"); on a GitHub mirror, Add from GitHub adds more from that repository (naming a different one switches it again) while Add from this computer is disabled. A door that cannot work for a project stays visible, shown disabled with the reason, rather than hidden — pressing it shows the same reason as a toast. Both doors open the same checklist: list what is there, tick documents (an already-added one is shown ticked and disabled; a file over 512 KB is shown disabled with its size), see a live total against the project's 200 KB budget, and commit with "Add N documents" or "Mirror N documents." A **Refresh** button ("Refresh from repo" or "Refresh from GitHub") sits beside the doors once the project has a source. Adding always appends — it never replaces a document that is already there. A document copied in through Add from this computer is labelled **"copied from `<folder>`"**, not "written by you," everywhere its source shows; editing it in the app's own editor makes it yours from then on. Add from this computer remembers the last folder a project added from, for that project, until the app quits — nothing is saved past the session.
...
- **Step ①, Documents** holds the canonical documents, or — before ownership is chosen — the question that chooses it. **Since v3.68.0 its head row always carries two doors, Add from this computer and Add from GitHub**, at zero documents and at twenty, matched to the project's one current source, plus Refresh (from repo or from GitHub) once a source is set, and, since version 3.67.0, **Suggest a reading plan** / **Suggest with AI**. Its table's SIZE column carries a small tinted bar behind each figure, showing that document's share of the 200 KB project budget, and its **At session start** column, since v3.67.0, is a real tri-state control — read first, on request, or not at start — see "How do I choose which documents an agent gets automatically?" above.
