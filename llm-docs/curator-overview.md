...
So the app measures it instead of promising it. Project context opens its Memory step with an **Agent connections** row (called *Agent sessions* through version 3.73.x and *Capture* through version 3.69.0; the on-disk name is unchanged), one line computed from a local, content-free log of which tools were called: how many connections in the last 30 days, how many started with the project's context, how many saved before stopping, and how many read and did not save. A connection is one MCP bridge process, not a conversation — Claude Desktop can hold many chats in one. It counts only connections that reached the project through the MCP bridge — a session started only by a lifecycle hook or `my-curator context` at the command line is not counted. When there is nothing to report, the row says why rather than reading "0 connections": "no usage log" if this computer has no MCP usage log at all, "not logged" if saves exist but none was recorded through the bridge; the "last 30 days" figure appears only once a count was actually taken. It is stated in words, never as a percentage, and it reports rather than blocks.
