...
<p align="center">
  <img src="docs/images/curator-agent-memory.png" alt="The Curator's Agent memory view: a Projects list whose rows each carry the project name, its newest headline and a freshness dot — field-notes 1 scope 1 day ago, projects 1 scope 2 weeks ago, lumina 22 scopes 5 days ago and curator 13 scopes 2 weeks ago, selected. On the right, a Status block leading with Working on and the newest save's headline beside a live age of 3 hr ago, then Last saved 2 weeks ago naming the work-stream curator-v3-17-1-acceptance and the harness that wrote it, then lines saying the file arrived on this computer 3 hr ago and that the reading above is the agent's own clock, that newer state exists in another work-stream of the same project, that the handoff was written on another machine and synced here, and the standing brief's age. Below it a Work-streams block: a table of the project's saved copies ordered newest first, each row a freshness dot and slug, that save's own headline, its age, the machine and the harness and model that wrote it — 3 hr ago and 5 hr ago in amber at the top, cooling through grey to hollow grey at 2 weeks ago"><br>
